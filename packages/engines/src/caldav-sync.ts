...
// vdirsyncer wrapper for CalDAV sync.
// Uses vdirsyncer for CalDAV synchronization with ledger-gated idempotency.
...
/**
 * Result of a CalDAV sync operation.
 */
...
/**
 * Generate a unique config file for a CalDAV sync job.
 */
function generateVdirsyncerConfig(config: CalDAVSyncConfig): { configPath: string; sourcePath: string; targetPath: string } {
  const jobId = createHash('md5').update(`${config.sourceUrl}-${config.targetUrl}-${Date.now()}`).digest('hex').substring(0, 8);
  const workDir = path.join(tmpdir(), `vdirsyncer-${jobId}`);
...
`);
  ...
  return {
    ...
    sourcePath: path.join(workDir, 'source'),
    targetPath: path.join(workDir, 'target'),
  };
}
...
/**
 * Run a CalDAV sync using vdirsyncer.
 */
export async function runCalDAVSync(config: CalDAVSyncConfig): Promise<CalDAVSyncResult> {
 ...
 */
export function cleanupCalDAVConfig(configPath: string): void {
  try {
