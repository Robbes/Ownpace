...
| A new token for a connection that already exists, minted through its provider's consent: the Connections row's button where the kind has a consent button, Google, Dropbox or Microsoft (workplan 0140 T2 (b)) | Reconnect | **Opnieuw verbinden** | The same panel as *Replace credentials* / *Inloggegevens vervangen*, which every other row keeps. The label follows the connection's kind, not what it stores: a Gmail row that signs in with an app password says *Reconnect* too. So *failure.authExpired* names both buttons verbatim, in each language, and leaves the choice to the row (*whichever its row shows* / *welke van de twee er bij dit account staat*), never tying a button to a password: it sends somebody to that page, and a sentence that names a button their row does not show sends them looking. Not *herstellen*: the Dutch sentence said *Herstel de verbinding*, which no button said either. |
...
| What Ownpace does at a source, and what the provider's permission allows (workplan 0144 T3) | only reads / read-only | **leest alleen** / **alleen-lezen** | Two facts, never one word. *Only reads* / *leest alleen* is the software's promise and true of every source. *Read-only* / *alleen-lezen* (*Alleen lezen.* where it opens the grant page's box) describes a source's PERMISSION, and is said only where the provider enforces it: Google Drive, Google Tasks, and Microsoft through *Connect with Microsoft* (not Microsoft 365 through IMAP, whose permission can write). For Gmail, Google Calendar and Google Contacts the permission also allows changes, and the grant page and the line beside *Connect with Google* say so. The permission is *toestemming*, as on the grant page (*grant.scopeIntro*). **Where the row holds today**: the grant page's box and its ending, the line beside *Connect with Google*, and `docs/grant-links.md`. **Still to follow it**, with whose they are: the site's how-it-works (0144 T3 (b), the owner's copy) and *setup.google.consent_scope.title* (0144 T3, after the first invitation). **Not yet decided**, and left as they are until they are: the software-sense uses, which mean what Ownpace does and would say *only reads* under this row (*discovery.scanning*, *wizard.testConnections.hint*, *verify.intro*, *connections.intro*), and the Dropbox and Box uses, where read-only is how the person's own app is set up rather than a scope the provider enforces (*wizard.about.dropbox* and its *.more*, the Dropbox and Box proto hints, *setup.dropbox.scopes.title*, *setup.box.configure_access.title*; Dropbox's consent is 0140 T7's). A role (*tenants.readOnly*) and a folder at the destination are other senses of the word, and not this row's. |
