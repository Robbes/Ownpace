...
describe('every 500 carries a reference', () => {
  ...
  it('gives each operating queue its own code, not one shared operating_failed', () => {
    // Owner decision, workplan 0081 T6. These twenty-six routes serve seven
    ...
    // announcement — 0104 T1/T3. Twenty-second: starting a confirmation pass,
    // 0117 T2 slice 7 — its own code because a confirmation that would not
    // start is a different conversation from a verification that would not.
    ...
    // question anybody will ask about it. Twenty-sixth: the FOLDER press,
    // 2026-09-19 — its own code beside the one-go press because the two are
    // different decisions about blast radius, and a fault in one says nothing
    // about the other: the folder press also runs a confirm-first gate the
    // one-go press has never had.)
