...
| **run-rollback.ts** | `tenantId`, `mappingId`, `reason`, `options.restoreDns`, `options.notifyUsers`, `options.dnsDomain` | Loads cutover state. Steps 1 (DNS restore) and 2 (data source restoration) are TODOs. Step 3 (state transition) works. Step 4 (notification) **is built as of 2026-08-03** — workplan 0030 T4: `notifyUsers: true` sends through the real channel, and is refused up front when no SMTP is configured rather than rolling back and telling nobody. | DNS rollback via DesecProvider; data source restoration; user notification |
