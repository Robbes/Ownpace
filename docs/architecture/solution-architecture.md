...
**v1.7 change:** mechanical corrections the code and later ADRs already settle — the title carries the one name (ADR-0040); Zitadel is the issuer (ADR-0042), not "Zitadel/Keycloak"; §17's secrets and threat-model rows say what SECURITY.md and ADR-0037 say (AES-256-GCM under `SECRET_ENCRYPTION_KEY`, **no vault**); §22.1's migrations are hand-written SQL in two chains (ADR-0045, ADR-0036), not Drizzle Kit; the adapters row names the sources that exist and drops Proton (ADR-0025, no code); Pattern D sits under *Partial* and Pattern S migrates (0027 T3), as the scope manifest has said since 2026-08-04; the release channels are the ones `images.yml` publishes.
...
One all-in-one bundle in several packagings: **Docker Compose** (NAS/mini-PC/Pi/laptop), ~~a **Home Assistant add-on** (Supervisor-managed), and an optional **hybrid agent** registered to the managed control plane that executes locally so the operator never sees content.~~ **Update 2026-08-05 (owner decision, 0026 T3 row 17): THE HOME ASSISTANT ADD-ON AND THE HYBRID AGENT ARE RETRACTED.** Both were deferred in workplan 0010 *until the compose path is proven*; it is proven, and zero code was ever written for either. The add-on is a **distribution channel, not a capability** — the appliance already runs on any Docker host, including a Home Assistant box, so an add-on buys convenience for one audience while the product's own first-run story is still unfinished (nobody has yet run the appliance on Windows). The **hybrid agent is a second architecture**, not packaging: an agent registered to the managed control plane and executing locally is a third deployment topology with its own credential flow, its own failure modes and its own security boundary — that is a product decision, and one nobody has asked for. Revisit condition, and it is a real trigger rather than a date: somebody running Home Assistant asks for it. Docker Compose is the self-host packaging. Because everything is idempotent + delta, an intermittently-on host resumes cleanly. No heavy orchestrator runs here. (This is about *where the tool runs*; it is **not** self-hosted email — targets remain managed EU/CH platforms.)
...
| Adapters/connectors | Microsoft 365, Google, Apple iCloud, Dropbox, Box, export archives, IMAP/JMAP/CalDAV/CardDAV/WebDAV (Proton deferred, ADR-0025 — no code); see `docs/feature-matrix.md` | idem | idem |
...
flowchart TB
  subgraph CP["Control Plane"]
    ...
    IDP["Identity (Zitadel, plain OIDC — ADR-0042)"]
...
**Update 2026-08-04 (workplan 0027 T1/T4, 0028 T3) — what of this now exists.** **Discovery does.** Mail-enabled groups are enumerated from the source directory, classified by the store signal exactly as this section prescribes (`Unified` → has a store → Pattern S; mail-enabled and not Unified → no store → Pattern D), and written to `group_def` with their member lists. The classification is **refused rather than guessed** where the directory does not say: the address is recorded with no pattern and raised as a `shared_address_pattern` decision, in this section's own words ("do recipients jointly handle one shared mailbox, or should it work as a distribution list?"), and the owner's answer is written back. That is §11.2's design working as intended — the S-or-D question was always meant to be asked. The discovered addresses appear on Review & confirm before anything is copied. **The migration itself does not exist yet.** Copying a Pattern S store as a mapping is workplan 0027 T3; recreating a Pattern D group on the target is 0027 T2. The scope manifest moved both rows from *Migrates* to *Partial* on the same date to say so, and they move back when those tasks land. **Two honesty properties hold throughout:** a source that cannot enumerate groups (every IMAP source; a Graph connection without application permissions) reports that it could not look rather than returning an empty list, and a group whose member list could not be read is stored as *members unknown* rather than as an empty group — because Pattern D recreates a group from exactly that list. **Update (0027 T3):** Pattern S now migrates as an ordinary mapping (`pattern: shared_s`); Pattern D stays discovered-and-guided (scope manifest, *Partial*).
...
**Update 2026-08-04 (workplan 0029) — what of this now exists.** **Discover, map and guide are built; apply is deferred by owner decision and has no code.** The report is generated on demand per mailbox (`/permissions/report`, both editions, Markdown) and reaches the owner on the Finish checklist, which is when it matters. **The most important thing it establishes is a limit:** FullAccess, SendAs and SendOnBehalf are Exchange recipient permissions and the Microsoft Graph API this stack speaks does **not** expose them — they are reported as *uninventoried*, every run, naming `Get-MailboxPermission` and `Get-RecipientPermission` as the way to capture them, because those are exactly the rights whose silent loss on cutover day this section exists to prevent. What IS read: calendar sharing, and OneDrive/SharePoint sharing including *anyone-with-the-link* grants, which are held apart from per-person grants because they are a different risk. The mapping step is **a table, not a translator**, honouring this section's own "without a fragile full ACL translator": two verdicts, clean or manual, no confidence score, and an unrecognised right falls through to manual rather than being guessed at. `clean` names what a right corresponds to on the target; it never means anything was created. The drive scan is bounded and **says when it stopped** — a partial inventory presented as complete is the report that gets an owner to cut over believing nothing was missed. The scope manifest's Permissions row was corrected the same day: it had promised that "only the clean, reversible subset is auto-applied" while nothing was inventoried and nothing applied.

**Update 2026-08-04 (the scope decision) — one of the two readable categories is switched off by default, on purpose.** This section names OneDrive/SharePoint sharing as part of *discover*, and the code to read it exists and is tested. What it needs is `Files.Read.All`, and that scope is unlike every other permission this product asks for: an Exchange **Application Access Policy** — §17's least-privilege mechanism, and the thing that makes application permissions acceptable here at all — is an Exchange mechanism and does not reach SharePoint. `Files.Read.All` has no narrowed variant, so granting it means read over every file in the tenant, with nothing able to fence it in. **Declined for the reference deployment (owner decision 2026-08-04)**, and `Calendars.Read` granted, which the same policy narrows to the mailboxes already in scope. The drive scan therefore sits behind `GRAPH_FILES_READ_CONSENTED`, defaulting off, and off means the report carries a **stated blind spot** naming the scope and the decision — never a missing section and never an implied *nothing is shared*. A deployment that has granted the scope turns it on with no code change. Revisit when the file migration itself needs the grant, at which point it pays for the migration rather than for a report about it.
...
**Secrets & access:** per-tenant OAuth tokens/credentials encrypted at rest (AES-256-GCM under `SECRET_ENCRYPTION_KEY`; a vault is not built — ADR-0037, SECURITY.md), least-privilege, token refresh, revocation on offboarding. Platform auth via an external OIDC issuer — Zitadel, self-hosted and replaceable by design (ADR-0042); tenancy and role come from `tenant_member`, never from token claims.
...
- *Self-host:* update via image tags — `edge` (every merge to main), `sha-<commit>`, `X.Y.Z` per release tag, and `latest` only from the first non-prerelease (`deploy/selfhost/README.md`) — pinned by digest; migrations auto-run on start (locked, idempotent); documented guidance: **back up the ledger before upgrading** and **never run two app versions against one database**; migrations are linear/cumulative so **skipping versions (N-2 -> N) is supported**.
...
**Every decision, with its CURRENT status, is in the register: [`docs/adr/README.md`](../adr/README.md).** This section used to restate all twenty-nine in prose and drifted from them — on 2026-08-13 it still described ADR-0027 as a Windows Service eight days after it became a scheduled task, and listed two retracted ADRs as live decisions. One register, linked, rather than two that disagree.
...
1. **API terms & app certification** — ~~finalise Microsoft Publisher Verification (and any 365 App Compliance/attestation) for the multi-tenant app~~ **resolved by decision 2026-08-09 (0026 T3 row 14): there is no multi-tenant app to verify.** Each customer registers their own Entra app in their own tenant and consents to it there — first-party consent, which Microsoft never gates on publisher verification. The duty this creates is documentation, not paperwork: `docs/o365-application-access.md` gains a guide-from-zero section so a cold-start customer can create the registration and both credential flows. Proton/Soton terms confirmation stays open as-written. (Legality of own-data migration is confirmed.)
2. **DNS management & email deliverability/reputation** — **decided 2026-08-09 (0026 T3 row 15): GUIDE, not automate.** The automate half is retracted, consistent with the 2026-08-05 deletion of the DNS write path (row 20): the product's DNS posture is verify-only, `docs/dns-management.md` is the deliverability story (SPF/DKIM/DMARC records, MTA-STS, an example TLSA entry), MX cutover is a manual operator step and IP warming is process advice. One extension parked behind a named trigger — teaching the read-only DNS verifier to confirm the guide's records (MTA-STS policy reachable, TLSA present) at the first real cutover where deliverability bites.
3. **Windows-native packaging** — decided since v1.1: **ADR-0027** picked a Start-menu shortcut over a native shell (Tauri deferred with a named revisit condition), and the runtime is already binary-free (pure TS engines + PGlite, ADR-0028). **Amended 2026-08-07: a Windows SCHEDULED TASK, not a Service** — hunting for an absent Services-panel entry cost real debugging time on hardware, so the wording matters. Execution is [workplan 0015](../workplans/0015-native-windows-installer.md). **Registration is done and proven on real Windows** — `scripts/windows/install-task.ps1`, boot survival, a 510-message migration and two hard kills mid-pass (2026-08-09), and an in-place payload upgrade over a running install carrying a live schema migration (2026-08-13). **What remains is the MSI and code signing**, in that order. (ADR-0019, ADR-0027)
4. ~~**Knowledge-enrichment add-in (OKF)** — optional, opt-in, **local-only** parallel `KnowledgeSink` that emits an **OKF** bundle (markdown + YAML frontmatter; later JSON-LD/RDF) describing the migrated corpus; deterministic metadata first, LLM enrichment a further opt-in layer; never on the migration critical path.~~ **Retracted 2026-08-05 (0026 T3 row 16)** — ADR-0021 is retracted and this is no longer backlog. Left struck rather than deleted, like items 1 and 2 above. (ADR-0021)
