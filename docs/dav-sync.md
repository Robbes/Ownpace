...
| The Nextcloud target answers nothing at all — `Empty reply from server`, no status code | PHP segfaulted on the request. `docker logs ownpace-nextcloud` shows `AH00051 … Segmentation fault`; nothing reaches `nextcloud.log`, because a crashed child never writes its access line. Since 2026-09-13 `deploy/compose/nextcloud-php.ini` disables the tracing JIT that caused it — see *When it goes wrong* in `docs/managed-bring-up.md` for the bisection |
