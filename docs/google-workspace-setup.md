...
That second question used to decide whether a format could carry a given kind of file. A
Google Doc has no bytes; exporting one produces a rendering, and this product hashes what it
writes, so while a rewrite followed the bytes, two exports that differed made every pass see a
changed document and copy it again. Now a document is copied again only when Drive says it was
edited, and a renamed one is paired by its Drive id, so every format carries every kind and
nothing is refused for what it measured. The table
below is what the measurement found on one real tenant, kept as a record. Run it on your own
Drive if you want your own answer.
...
Then keep a note of what you found.
...
| --- | --- | --- | --- |
| **Doc** | **differs between exports** | the same | the same |
| **Sheet** | the same | the same | the same |
| **Slides deck** | the same | **differs between exports** | the same |
| **Drawing** | the same | the same | the same |

Two cells differ, for one reason: two exports of the same unchanged file do not come back byte
for byte the same. A Doc's `.odt` rewrites its `settings.xml` (view settings such as zoom and
cursor position, not the text), and a deck's `.pptx` changes its relationship files and themes.
Until 2026-09-23 those two were left behind, because copying them meant copying them again on
every pass. **They are copied now**: a document is copied again only when Drive says it was
edited, and a renamed one is recognised by its Drive id, so the bytes decide nothing. Several of the other cells come back as the same content in a re-stamped zip, which
this product compares by the document's parts rather than by the whole file, so a re-stamp is
not a change; that is what "the same" means here.
...
recommendation: if you want to go on editing your files you want OpenDocument or Office, and PDF
if you only need to keep what they look like.

**What "the same" is and is not.** A difference settles a cell: one counterexample answers the
question. A pass is weaker —
