...
| [0036](./0036-managed-only-code-lives-in-its-own-package.md) | Money lives in its own package, and the appliance cannot reach it | **Accepted 2026-08-19** (owner decision) — draws ADR-0003's edition boundary where it had never actually been drawn. Pricing, tenant pricing, usage metering and the `invoice`/`payment_method`/`usage_metric` tables leave `@openmig/shared` and `@openmig/ledger` for a new `@openmig/managed`; the billing screen leaves the appliance's UI bundle. The leakage guard passed throughout because its rule was the word "billing" and these modules were called `pricing` and `usage-metering` — it now forbids those, and gains a schema half. `tenant.pricing` and the baseline's billing DDL deliberately stay (hard rule 2; `offboarding.ts` needs `invoice` to exist), named rather than tidied. Two-repo options stay parked and get easier, not harder |
