...
| [0046](./0046-a-rendering-is-compared-by-its-parts.md) | A rendering is compared by its parts, not by its bytes | Accepted 2026-09-16, on the 0042 T3 measurement that `export-office` is unstable only in its zip container. **Built (0042 T7), not yet reaching the ledger**: exports are still stored with a whole-file hash (0042 T8 (e)) |
