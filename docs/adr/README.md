...
| [0034](./0034-appliance-configuration-surface.md) | Personal / Organisation / Managed — naming the deployments, and giving each the configuration door it needs | **Proposed 2026-08-17; open questions RESOLVED by the owner 2026-08-19** — after a same-day correction: both had been recorded as delegated on a *no preference* the owner never gave (an unanswered picker read as consent). The answers reshaped decisions 2–5 into a split **by concern, not by deployment**: topology's door differs (files are Organisation's sole door, UI is Personal's, API is Managed's) while credentials and grants live in the one store everywhere — which deletes decision 4's ownership/collision machinery outright. Key/provider/TLS mechanics moved to ADR-0037. The full accept/reject of the surrounding reasoning is still open. Names two axes the tree conflates: **deployment** (Personal / Organisation / Managed, orthogonal to ADR-0003's edition) and **configuration surface** (Declared = files, Operated = UI, per object and never merged). Files scale UP (an Organisation deployment's ~1000 end users are declarative), the UI scales DOWN; the encryption key is generated for Personal and comes from a secret manager for Organisation; **authentication is a prerequisite** before the credential-editing UI reaches a non-loopback bind. Written because workplan 0066 recorded "the appliance's connections come from mapping files" as settled architecture when it was only workplan 0010's implementation choice — and because one word for two deployments is what let that happen. Flags that SAD §3/§4.1/§7/§7.1/§8's "hobbyist / single-user / single tenant" language contradicts the owner's stated scale |
| [0035](./0035-who-signs-in-and-who-gets-a-link.md) | Who signs in, and who just gets a link | **Proposed 2026-08-17** — the substance is owner-decided (only the migrated person holds their own source credential; migrated people get links, not accounts; admins see their whole family/organisation's progress; billing answered inside). Two populations: `tenant_member` rows sign in, mappings get links. Names three credential categories and admits the organisation-held one (Box CCG, app-only Graph, Google DWD) cannot be narrowed. **Restates ADR-0034 decision 6** — a link is not a login, so "not a thousand interactive logins" still holds. Owner restated 2026-08-19: owners log in; the owner decides who gets a link to manage and grant their own migration |
| [0036](./0036-the-managed-edition-is-its-own-package-and-its-own-chain.md) | The managed edition is its own package and its own migration chain | **Accepted 2026-08-19** (owner decision, in two rounds) — draws ADR-0003's edition boundary where it had never actually been drawn. Pricing, tenant pricing, usage metering, offboarding and seven tables leave `@openmig/shared` and `@openmig/ledger` for `@openmig/managed`, whose DDL is a second migration chain the appliance does not apply; the billing screen leaves the appliance's UI bundle. The leakage guard passed throughout because its rule was the word "billing" while the modules were called `pricing` and `usage-metering`. `tenant.pricing` and the closure dates became ROWS rather than columns; `tenant.status` keeps `closed` because a constraint says what is allowed. **Records three of its own claims being wrong** — offboarding is never executed on the appliance, the column did not have to stay, and a shared ledger breaks whichever chain a filename happens to sort against. Compressing the 27-file shared chain is REJECTED until a guard compares RLS policy definitions. Two-repo options stay parked and get easier, not harder |
| [0037](./0037-keys-credentials-and-transport-floors.md) | One credential store, two key providers, and TLS floors | **Accepted 2026-08-19** (owner decisions; completes ADR-0034's resolution) — credentials and grants live in the one `SecretStore` in every deployment, because ADR-0035's grant links arrive from people at runtime and have no env var; the four env-indirection auth kinds stay per connection as the no-secret-at-rest escape hatch. One `KeyProvider` seam, exactly two providers: env (always wins; Organisation's secret manager, Managed's platform injection later) and a generated file for Personal on **every** platform — OS keystores deferred, because a Linux keyring exists on the desktop and not on the home server, which would fork one OS into two postures. The key is required at **first store**, not at boot, so today's files+env fleets gain no knob. AES-256-GCM stands (no PQC theatre; ~128-bit quantum margin); the blob's version byte becomes a **dispatcher**, which is the whole crypto-agility mechanism. TLS: named cloud providers get a 1.3 floor; customer/legacy endpoints get 1.2 restricted to ECDHE+AEAD with the negotiated version surfaced — a hard 1.3 floor everywhere would refuse exactly the servers people migrate off, and that adjustment of the owner's instruction is flagged in-file for overrule. Managed's plaintext `.env` key is **named as the open gap** |
