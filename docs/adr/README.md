...
| [0032](./0032-sharing-queue-target-native-invites.md) | The sharing queue — re-sharing on the target as an owner decision, invites through the target's own messaging | **Proposed 2026-08-16** — §14.2's deferred write half: grants become queue rows, apply is per-grant and owner-decided, gated behind cutover; the target platform's own share notification is the message to the people affected (open-migrate never mails third parties itself) |
