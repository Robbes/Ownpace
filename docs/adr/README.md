...
| [0034](./0034-appliance-configuration-surface.md) | Configuring the appliance — the UI is a real door, files stay for fleets | **Proposed 2026-08-17** — awaiting an owner decision. The appliance gets the same connections/mapping surface the managed edition has, config files keep working unchanged, ownership is per object and never merged, and the appliance generates its own encryption key so a Windows user is never asked for one. Written because workplan 0066 recorded "the appliance's connections come from mapping files" as settled architecture when it was only workplan 0010's implementation choice |
