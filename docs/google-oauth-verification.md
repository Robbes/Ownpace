...
| Application home page, on the verified domain | 🟡 Built, not yet published on `ownpace.eu` | `site/build.mjs` renders it ([workplan 0091](./workplans/0091-the-names-on-one-box.md) T2; served noindex on `www.ota.ownpace.eu` by `deploy/compose/www.yml`); a `--public` build is refused while any `«PLACEHOLDER»` in `site/legal/` is unfilled |
...
`«PLACEHOLDER»` tokens for things only the owner can supply — registered address, VAT number,
hosting provider and region, email provider and region, log retention, and the sub-processor
and version-history URLs (the entity, KvK number and court district were filled 2026-08-30).
`site/legal/README.md` lists every one of them, and a unit test fails if a placeholder appears in
a document without being listed there.
...
3. **Publish the front door** — it is built (`site/build.mjs`); a `--public` build for
   `www.ownpace.eu` becomes possible once step 2 is done.
