...
| `[setup-zitadel] FATAL:` a call answered `HTTP 404` with `unable to set instance using origin` / `Instance not found` | **The provider does not serve the origin being presented.** Zitadel resolves which instance a request is for from the request's origin — host AND port — and refuses any other. That origin is fixed AT FIRST INIT from `ZITADEL_EXTERNALDOMAIN`, and there is no way to add one afterwards with a provisioning token: `AddInstanceDomain` lives on the System API (a PAT gets `401 Errors.Token.Invalid`), `POST /admin/v1/domains` does not exist (`404 Not Found`), and an instance TRUSTED domain does not change origin resolution — all three measured on a live instance | An instance initialised under a different domain has to be initialised again. That destroys the provider's own database and nothing else — no Ownpace data, no Trigger.dev account — and `setup-zitadel.sh` rebuilds the project, the application and the client id afterwards. The refusal prints the exact commands; they are the same four as the rows above, plus `up -d zitadel` |
