...
| T3 Live cutover on the Spark stack | ⬜ Not started — **needs the live stack** | Sequence below; acceptance = tick runs visible, item counts still moving, `smoke-managed.sh` PASS, with the poller STOPPED. |
| T4 Retire the poller | ⬜ Not started — **only after T3's evidence** | Remove the `worker` service from `managed.yml`, delete `managed-scheduler.ts`, update the runbook's division-of-labour note and 0021's untested-seams list. |
| T5 Docs truth pass | ⬜ Not started | Runbook scope note, `deployment.md` if it names the worker service, CHANGELOG entry. |
