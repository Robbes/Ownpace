...
the deployment's own client pair (`GOOGLE_OAUTH_CLIENT_ID`/`_SECRET`, both-or-neither),
resolved at every door, half a pair refused, the facts on `/api/provider-clients`, and the
managed gate asking the real stack all three questions with a sentinel pair (#729). What T5
still owes is the owner's: the verification submission itself. T8 was superseded rather than
built: instead of two named options on the credentials step, the pair FOLDS AWAY wherever the
deployment carries the client (#712), so a person never chooses — the owner walked it
2026-09-02. Rows updated below; nothing else changed.

| Task | Status | Evidence |
|---|---|---|
| T1 The authorization-code flow, against the customer's own client | ✅ **Done 2026-08-26** | Two routes and a wizard button, exactly as planned. `google-consent.ts` carries every load-bearing constraint as code, each proved by breaking: the `state` is an HMAC-signed, SINGLE-USE (take deletes — reusing one turns a test red), ten-minute id whose secret half waits in process memory and is never in a URL or at rest; `access_type=offline` + `prompt=consent` are pinned on the URL; the granted scope is READ off the token answer and a narrower grant refuses with the missing scope named (making it accept turns a test red); the result page postMessages the token to the configured web origin and never `*`, degrading to copy-paste when none is configured. The wizard's **Connect with Google** button sits beside the fields (which stay), waits for the client id+secret, and the popup's answer lands in the SAME field a pasted token uses — origin-gated (trusting any origin turns the reachability walk red), stored through the same encrypted path (ADR-0037, no special case). `docs/google-workspace-setup.md` steps 3–4 now lead with the button and keep the manual path; the "It is planned" sentence stopped being true and was replaced. **Follow-up 2026-09-02:** the page ran behind helmet's defaults — CSP `script-src 'self'` denied its inline script and COOP `same-origin` severed the popup's opener at the navigation back from Google — so the owner's walk ended on "Handing the result back to the wizard…" with nothing arriving, and no test had asked what the page was served under. The callback now answers with headers derived from the page (`callbackPageHeaders`: the script permitted by its own SHA-256, `unsafe-none` for the opener), proved with helmet mounted in `google-callback-route.unit.test.ts` and asserted on the deployed stack by the managed smoke. |
...
| T5 The managed client | 🟡 **Mechanics built 2026-09-02 (#703–#712, ADR-0041); the verification submission is the owner's** — ADR-0041 **accepted 2026-08-26** (as proposed; Drive's assessment intended later; the 2026-08-20 client is the test/OTA one, production splits off before real customers) | First gate **answered 2026-09-20** from the console's Data Access page: carddav and the read-only calendar scope are *sensitive*, Drive and every Gmail scope *restricted* — the cheap slice is cheap (`docs/google-oauth-verification.md` §2). Contacts and calendar first, mail and files only after an assessment is paid for. Managed-only secret; `no-managed-leakage` is the guard. The submission checklist is [`docs/google-oauth-verification.md`](../google-oauth-verification.md); privacy policy, terms, support address and logo are drafted (0086 T5). |
| T6 A callback for a box reached at an IP | ✅ **Done 2026-08-26** (the no-forward-no-DNS case stays open as planned, both candidates unverified) | `rawIpCallbackRefusal`: the authorize route refuses a non-loopback IP-literal callback BEFORE Google's screen, naming both supported shapes — forward a port and register `http://localhost:<port>/…` (loopback is permitted, and deliberately so: the remedy depends on it), or give the box a hostname (the objection is the IP literal, not the network) — and that the paste-a-token path keeps working meanwhile. The manual's redirect-URI step carries the same two shapes in the same words. | Google forbids raw-IP redirect URIs, so a box browsed at `https://100.64.0.1:3123` needs either a **hostname** (an `A` record at that address is legal — Google never resolves it) or a **port-forward to loopback**. Both are documentation, not engineering, because the appliance uses the customer's own client and so the customer registers their own URI. Only the case where neither is available stays open. |
| T8 "Use Ownpace's connection" as a choice in the managed wizard | ✅ **Superseded 2026-09-02 (#712): no choice to make — the pair folds away where the deployment carries the client** | The owner registered a client 2026-08-20. Two named options on the credentials step, the managed one only in managed builds. Config decisions in [`docs/google-oauth-verification.md`](../google-oauth-verification.md) §4b. |
...
**Answered 2026-09-20.** The owner's console sorts `.../auth/carddav` and
`.../auth/calendar.readonly` into *sensitive* and `drive.readonly` plus every Gmail scope into
*restricted*, so the cheap path is cheap. (There is no `.../auth/caldav` scope; the product asks
for `.../auth/calendar`, and whether its read-only sibling suffices over CalDAV is a Stage 6
question in the owner test runbook.) Copied as displayed into
`docs/google-oauth-verification.md` §2.
...
appliance at `https://100.64.0.1:3123`. So T1's callback needs somewhere to land.
