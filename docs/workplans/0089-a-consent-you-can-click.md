...
| T7 Personal Gmail by app password, opt-in | 📋 Planned — **unblocked 2026-08-26** ([0090](./0090-the-cap-we-do-not-count.md) T1 verified) | A credential choice, not a connector — the IMAP source already has the branch. Google's own *afgeraden* travels with it, and it is never the default. The ceiling is verified at **2 500 MB/day, equal across app passwords and XOAUTH2** (0090 T1), so this path adds no new exposure — and the sensible order is still to land 0090 T2–T4 first, so the budget exists before a second on-ramp points at the same uncounted cap. |
