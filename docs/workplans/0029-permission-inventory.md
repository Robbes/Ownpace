...
|---|---|---|
| T1 Discover: delegations + shares, read-only over Graph | 🟡 **The read layer is built 2026-08-04; the wiring that produces a report is the next slice** | **The finding worth leading with: Graph cannot read mailbox delegation.** FullAccess, SendAs and SendOnBehalf are Exchange recipient permissions, and the Graph API this tool speaks does not expose them — the way to enumerate them is Exchange Online PowerShell (`Get-MailboxPermission`, `Get-RecipientPermission`), a different credential, consent model and protocol. `mailboxDelegations()` therefore always returns `not_discoverable`, naming those cmdlets, and it is a tested function rather than an omission — because those are exactly the rights whose silent loss on cutover day this plan exists to prevent, and a report that skipped them would read as *nothing was delegated* (rule 9). **What IS readable**, both read-only and both behind 0027 T0's application-permission mode: calendar sharing (`calendarPermissions`) and OneDrive/SharePoint sharing (a drive item's `permissions`). Sharing LINKS are told apart from people-grants — *anyone with this link can edit* is a different risk from *Anna can edit*, and it is the finding an owner most often does not know about. Every failure path returns `not_discoverable` with the reason, never `[]`: no permission, an error status carrying Graph's own words, a transport failure, a non-JSON body, paging that will not end. The vocabulary lives in `@openmig/shared` (`PermissionGrant`, `PermissionListing`) for the same reason `DirectoryListing` does — the connector that produces findings and the report that consumes them are in different packages. `raw` carries the grant verbatim and is never parsed downstream: it is evidence, not a model. 13 tests. **Owner decision needed before this can run live:** the two Graph scopes it uses — `Calendars.Read` and `Files.Read.All` — are NOT in `docs/o365-application-access.md`'s consented set, and that runbook says adding a permission is *worth a conversation rather than a click*. This is that conversation. **Answered 2026-08-04: `Calendars.Read` yes, `Files.Read.All` no** — see the closing note on this plan. **The wiring landed the same day.** `scanCalendarPermissions` reads every calendar in a mailbox and merges the shares into ONE listing — a mailbox with eight calendars would otherwise produce eight sections of which seven say nothing — and fails the whole listing if any single calendar cannot be read, because *these are your calendar shares, except the ones we could not read* is the half-truth this module exists to avoid. `scanDrivePermissions` is where the awkward trade-off lives: a tenant's drive can hold a hundred thousand items, so it asks only about items carrying Graph's own `shared` facet (an unshared item's permission list holds nothing to act on — the difference between a handful of requests and a hundred thousand), and it CAPS both the folder walk and the permission reads. **A hit cap returns `not_discoverable`, never a short list**, because a partial inventory presented as complete is exactly the report that gets an owner to cut over believing nothing was missed. `resolveUserDriveId` is a separate step rather than a concatenated path, for the same reason `graph-scope.ts` validates rather than concatenates. 10 more tests. **The drive scan now sits behind a consented flag** rather than being deleted or left to fail: `driveSharingAvailability` reads `GRAPH_FILES_READ_CONSENTED`, defaults OFF, and off produces a STATED blind spot naming the scope and the decision. A flag because the choice belongs to whoever runs the product — a deployment that granted the scope gets the section with no code change and no fork. And a flag rather than letting the scan run and 403, because those two produce the same shape and opposite meanings: *Insufficient privileges* reads as a consent to go fix in the portal, and sends an operator on an errand with no end. 9 tests. **Not done:** live proof, which needs the consent runbook run on the test tenant. |
| T5 The Google half: Drive outbound shares | ✅ **Done 2026-08-16** | `GoogleDriveSource.listOwnedShareGrants()`: ONE paged `files.list` over `'me' in owners and trashed=false` with `permissions` riding along in the fields — Drive populates them on owned My-Drive files, so this never becomes the per-file crawl the Graph scan's second phase is, and it needs **no new consent**: the Drive scope a pass already holds reads them. The owner's own permission row is skipped (not a share); `anyone` grants are flagged `viaLink`; `raw` keeps Drive's fields verbatim, file id included. Capped like the Graph scan — a hit cap answers `not_discoverable`, never a short list dressed as the whole one; so does any failed listing (hard rule 9). Scope is exactly *files the account owns*: a shared drive's membership is drive-level and its files are owned by the drive, so nothing here speaks for shared drives, and the docs say so. Both editions wire it into the same report: managed branches on the tenant's source connection KIND (stored credentials via SecretStore), the appliance on a mapping's source type — same factory and env names as a pass. **Corrected 2026-09-17, and the correction is the finding:** for thirteen months each edition asked for the literal `google-drive`, and on the managed side that value has never been storable — `connection.kind` is CHECK-constrained and spells it `google_drive` (migration 0008); `google-drive` is the WIZARD's word. So the managed lookup matched no row for anybody, ever, and the appliance's matched only the legacy type, missing the `google` ACCOUNT kind Connect with Google creates. The owner found it the honest way: a live Google-to-Nextcloud migration full of shares whose Sharing page said nothing was there. Managed now asks `connectionKindsWithFace('file', 'google-drive')`, derived from the face tables, and the appliance asks the shared `carriesGoogleNativeFiles`. A second defect sat behind the first: a Connect-with-Google row stores a refresh token and NEITHER half of the client (ADR-0041, option B), so the credentials went to the builder and it refused for values that are deliberately absent — `resolveGoogleClient` now fills the deployment's pair in, as every other Google door does, and a refusal is a sentence rather than a throw. Three guards, each proved by restoring the defect: `a-kind-the-column-cannot-hold.unit.test.ts` reads the CHECK constraint out of PGlite rather than trusting a list, `a-share-scan-that-never-ran.unit.test.ts` runs the real resolver over a fake pool that filters by the query it is given, and `a-google-mapping-the-share-scan-could-not-see.unit.test.ts` holds the appliance to the shared list. A Google-only deployment's calendar section now states the honest Google sentence instead of a Graph-worded errand about an app registration it never had. 3 unit tests. **Not done:** live proof (rides the owner runbook's Google stages), and Google Calendar/Contacts sharing scans — stated blind spots, not silent ones. |
...
## The scope decision (owner, 2026-08-04)

Asked as one question and answered as two, because the two scopes are not
alike.

**`Calendars.Read` — granted.** It is an Exchange-family permission, so the
Application Access Policy in `docs/o365-application-access.md` step 4 narrows
it to exactly the same mailbox group that already narrows `Mail.Read`. It is
not new reach; it is one more data type over mailboxes the app can already
read.

**`Files.Read.All` — declined.** Application Access Policy is an Exchange
...
every site collection, with nothing in this runbook able to fence it in.
(SharePoint's own scoping mechanism, `Sites.Selected`, is a different
permission with a per-site grant model this code is not written against.) A
tenant-wide standing file read, bought for one section of a handover report,
is the wrong trade — and the section is *stated as unread*, not omitted, so
nobody is misled by leaving it out.

**Revisit when the drive migration itself needs file access**, because the
grant then pays for the migration rather than for a report about it. Nothing
has to change in the code first: `GRAPH_FILES_READ_CONSENTED=true` turns the
section on.
