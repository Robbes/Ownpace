...
| T4 API: discover + poll (managed) | ✅ Done | `packages/shared/src/scope-manifest.ts` (moved from `apps/api/src/services` so both editions share one manifest, no cross-app import). `apps/api/src/routes/scope-manifest.ts` — `GET /api/scope-manifest` (no auth; a global, public promise set, not tenant data). `apps/api/src/routes/migrations/index.ts` — `POST /:id/discover` (enqueues `run-discovery`), `GET /:id/discovery` (`PgDiscoveryStore` via `withTenantDb`, RLS). **Tests:** `discovery-routes.integration.test.ts` (7) — scope-manifest, discovery empty/populated, paused-sync 409, start idempotent, 404s. **Gates:** lint + typecheck + api integration green. 2026-09-08: `GET /:id/discovery` now filters the stored rows by the mapping's own `scope_selection` (`discoveryForSelection`, `packages/shared/src/discovery.ts`). The enqueue side learned this in #854; the read side had not, so rows written BY the defect #854 removed — a Tasks row holding the Drive numbers, on a mapping whose Tasks tick has never been on — kept coming back as part of what was about to be migrated. Filtered rather than deleted: it reaches rows already stored with no migration, and keeps the evidence of what that pass saw. Guard: `packages/shared/src/a-preflight-row-for-a-domain-the-migration-does-not-carry.unit.test.ts` (11), proved by breaking. |
...
| T9 one count per migration, and a reload joins it | ✅ **Done 2026-09-22** | The confirm screen enqueues a preflight when it opens, and its slow message says *"reload to check again"*. `POST /:id/discover` enqueued with nothing to tell one request from the next, so every reload started another full count beside the first, against the same provider. On the owner's first Microsoft run, calendar and contacts were re-counted mid-flight by a count that had just begun. `discoveryTriggerOptions` now sets two keys. The **idempotency key** covers the migration, its `updatedAt` and the domains asked for, so a reload is the same count and Trigger.dev returns the run already started, for 15 minutes, while an edited migration gets a new count at once. The **concurrency key** is the mapping, and `run-discovery`'s queue allows one run per key (the shape `run-delta-sync` uses), so two counts of one migration never race. Pinned by the rule (`a-count-each-reload-started`), by the real route over an in-process ledger (`the-discover-route-joins-a-running-count`), and by the queue's limit (`one-count-per-migration`); seven mutants, all killed. |
