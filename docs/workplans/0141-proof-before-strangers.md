...
consent screens, and nobody else holds those (D5). A few related items are fixed in #1137 (pending
merge), and each is named where it comes up. Every fact in §1 was re-checked at the current
checkout on 2026-09-24. The run histories were read from GitHub's API the same day.

**Before the first invitation.** This is the minimum, and it is kept small:

- T1, the record;
- T9, the organiser canary;
- T12's walk with two strangers;
- T14's reading of the nightly gate;
...
| T12 The managed journey in a browser | ⏳ **Owner** (the walk); 📋 **Proposed** (fixture cases); 🅿️ **Parked (trigger: 0132 T8)** (the real-API journey) | §3. The two-stranger walk happens **before the first invitation**. |
...
| T14 The nightly managed gate's readiness rule | ⏳ **Owner** (names N); 📋 **Proposed** (the rule) | §3. Worked out with 0132 T1. It is read **before the first invitation**, and before the gate is switched off. |
...
|---|---|---|---|
| IMAP (`imap`) | mail | proven on a server we run | Driven every night against Stalwart (`scripts/connector-coverage.unit.test.ts`:71). |
...
| Google account (`google`) | calendar, contacts | proven on one account | *"run against the owner's real Google account routinely for weeks"*, and *"NOT … a SECOND account"* (matrix :10-16). |
| ″ | tasks | experimental | 0126 T8 is ⏳ (0126:71). The matrix's gap row marks it ✅ and *"unmeasured against a live account"* (:406). |
| ″ (whole-domain option) | all | experimental | *"Real-endpoint proof — DWD-minted tokens against live Drive/Gmail/Calendar/Contacts"* is ⛔ (0053:10). Matrix :400. |
| Google Drive, Gmail, Google Calendar, Google Contacts | — | proven on one account | Matrix :29, :78, :181 and the Files table. Drive: owner runbook Stage 1 *"Done 2026-09-16 and 17"* (`owner-test-runbook.md`:52), and 0042 T8, *"what the first live run left open"*. An edit made in Drive reaching the copy is *"proven in tests, not yet seen on a live run"* (0042:82). |
| Dropbox | files | experimental | *"(a) Real-endpoint proof — ⏳ … unproven here"* (0055:11). Matrix Files table ⏳. |
| Box | files | experimental | The same words in 0056:10. Matrix Files table ⏳. |
| Apple account (`apple`) | mail, calendar, contacts, tasks | experimental | *"Still **not measured against a live iCloud account**"* (0115:68-69). Matrix :403. Part 1 of `docs/apple-supervised-run.md` is still open. |
...
| Card | Face | Today | What it rests on |
|---|---|---|---|
| JMAP | mail | proven on a server we run | Stalwart, every night (`connector-coverage.unit.test.ts`:149). |
...
| IMAP | mail | proven on a server we run | `imap-dav` is driven against Stalwart on every pull request (`connector-coverage.unit.test.ts`:153-157). |
| CalDAV, CardDAV, WebDAV | — | proven on a server we run | Nextcloud, every night (:150-152). |
| Nextcloud (`nextcloud`) | calendar, contacts, files, tasks | proven on a server we run | The managed gate's smoke writes to the demo Nextcloud (`smoke-managed.sh`:4518, :4556). 0103's run of 2026-09-22 was the owner's live Google → Nextcloud migration. A Nextcloud somebody else runs has not been a target. |
...
*"unmeasured against a live tenant"* and *"unmeasured against a live account"* (:405, :406). The
open-gaps table does not list Dropbox and Box; that is fixed in #1137 (pending merge).
...
- GitHub records two runs, both dispatched on 2026-09-06 and both `cancelled`. The review found
  ...
  `runs-on: [self-hosted, linux, arm64, spark]` (:85), while every self-hosted job that works asks
  for `[self-hosted, linux, arm64]`. #1137 leaves the label to the owner, because the job reads a
  real tenant and the pin may be intended.
- The soak switch sets `SOAKE_TEST_24H` (:81), while the suite reads `SOAK_TEST_24H`
  (`o365-scenario.e2e.test.ts`:155). This is fixed in #1137 (pending merge).
...
describes as *"a long-lived token from a real issuer"*.

**3. The coverage table cannot speak for most of what a tester picks.**

- `connector-coverage.unit.test.ts` marks `microsoft`, `google`, `gmail`, the three Google product
  types, `dropbox` and `box` as `uncoverable` (:98-122), and the five `graph-*` types as `owed`
  (:82-97).
- Its lists come from what `config.ts` accepts (:57-66). The managed account kinds `apple`,
  `soverin` and `nextcloud` are not config types (`config.ts` has a branch only for
  `type === 'microsoft'`), so no verdict speaks for them at all.
- Its one `jmap` target verdict reads *"the mail target every nightly writes into"* (:149) and
...
- Drift detection finds a tenant's Microsoft source with `kind = 'o365'`
  (`managed-drift-detect.ts`:103-104). It then asks `directoryAvailability(process.env,
  graphTenantId)` (:115) and mints the token with the stack's client for the customer's tenant
  (:119-124). The permission report does the same lookup and the same call (`permissions.ts`:259-265).
- `directoryAvailability` reads only `OAUTH2_CLIENT_ID`, `OAUTH2_CLIENT_SECRET` and
  ...
  (`managed.env.example`:58-61), uploaded to the tasks by `set-task-env.sh` (:184-187).
- Only the `oauth2` and `graph` cards are stored as `o365` (`credential-fields.ts`:641-643). A
  ...
  Graph can enumerate a directory"* (`directory-availability.ts`:45-52). Group discovery gives
  every non-`o365` source IMAP's answer (`managed-group-discovery.ts`:102): *"IMAP has no directory
...
- Who reads these sentences:
  - The two jobs write their blind spots to the worker log (`run-new-mailbox-detection.ts`:99-103),
    so the operator reads the wrong reason.
  - The permission report is read by the tester. The Finish step's `PermissionsHandover` fetches
    `/permissions/report` (`operating-service.ts`:211), and the report's calendar section carries
    the sentence.
- The true answer for a `microsoft` connection would be different anyway. Its grant is delegated:
  `Mail.Read` over the signed-in person's own data (`microsoft-scopes.ts`:53). No stack credential
...
#1137 (pending merge) passes `GRAPH_FILES_READ_CONSENTED` to the API, which the report reads. It
...
- Grant links are Google only (`grant.ts`:273-274, `/:link/google/authorize`).
...
- #193 and #194: the tasks' Node runtime, fixed by 65c0926;
...
#1137 (pending merge) moves the managed gate and the lane to Node 24. Its description asks for one
dispatched run after the merge. 0132 T1 proposes switching the scheduled gate off for the alpha.
0131 T5 asks for N green scheduled runs of the deployed commit.
...
read as "CI stays".) So a proof runs on the OTA stack, through the managed wizard, the way a tester
will. CI's lanes run on the same machine.
...
1. **The product's own path.** The managed wizard on the OTA stack, or the appliance with a mapping
   file. The run uses the product's own connector for that kind and face. A harness with a client of
...
It fails today: the section does not exist, and the open-gaps rows at :405 and :406 break the
...
**The sitting.** It runs on the OTA stack. The owner uses *Connect with Microsoft* with their own
account, the one 0114's Test measured, and ticks all five faces. Calendars, contacts, files and
tasks go to the owner's Nextcloud, and mail goes to a mail target the owner controls. The faces go
in this order:
...
deployment's registration, and it records the consent side. If its pass completes, it is also a
...
**Dropbox, before a tester who uses it.** The deployment needs a real Dropbox app first. 0140 T7(a)
asks whether the reference machine's `DROPBOX_OAUTH_CLIENT_ID` is a real app or the gate's
placeholder, and 0132 T5 removes placeholders. The sitting:

- the owner's own Dropbox account, holding a tree with nested folders;
- the target is the owner's Nextcloud;
...
2. **The first pass.** Part 1 stops at Test on purpose: *"a first real run against a personal Apple
   ...
   calendars, contacts and Reminders to the owner's Nextcloud, and mail to a mail target.

Check the two hosts and the home set on the partitioned host (matrix :120-123), and Reminders
arriving as `VTODO`. Record in 0115 and in the matrix row at :403. The guide says the owner has an
...
- **Drive, on the owner's own account:**
  - an edit made in Drive reaches the copy on a live run (0042:82);
...
0105 T3's sitting, `docs/soverin-supervised-run.md` steps A to H. Three things change first:

- **Step H's sign-in.** Mint mode cannot sign in to the OTA stack (§1). Step H is rewritten for
  `LIVE_TARGET_API_TOKEN`: whose token it is, how it is minted, and how long it lives. That is
  written into the runbook before the sitting.
- **The source.** Step B seeds the reference machine's demo Nextcloud as the source (*"it plays the
  SOURCE, because we may seed only what we host"*). 0132 T5 removes that Nextcloud and its volume.
  Either the sitting runs before 0132 T5, or it seeds a source that survives it (open question 3).
- **T9's event.** Step B gains one event whose organiser is the Soverin account's own address, so
...
- The appliance nightly (`e2e.yml`, Stalwart) gets a leg that maps contacts and files to `jmap`.
  0132 keeps that nightly running during the alpha.
...
1. Set up the owner's Nextcloud target with its SMTP pointed at a catcher, as 0103 T2 arms the
   demo one.
...
organiser is the demo target user's own address, so the nightly proves it again once the gate runs
again. **Guard:** a rule in `scripts/the-mail-nobody-should-get.unit.test.ts`: the smoke seeds an
event whose `ORGANIZER` is the target account's own address. It fails today, because T2's fixture
has only a third-party organiser (0103:20).
...
place for the run. The alpha stack keeps `OAUTH2_*` empty (T11).
...
- both jobs are scheduled in code (`cron: '0 7 * * *'` in `managed-drift-detect.ts`:69,
  `'30 6 * * *'` in `managed-group-discovery.ts`:64), and no setting stops them;
...
- The three lookups ask for every Microsoft kind, not `kind = 'o365'`. They use
  `connectionKindsWithFace('email', 'graph-mail')`, the way `permissions.ts` already asks for Drive
  (:286-294, after the defect its comment describes).
- Group discovery gives a `microsoft` source that reason, not IMAP's.
- On the alpha stack, `OAUTH2_*` stays empty. That check belongs in 0132 T0's list. For an `o365`
...
- `scripts/a-microsoft-source-found-by-its-faces.unit.test.ts`: no SQL in `apps/` finds a tenant's
  Microsoft source by `kind = 'o365'` alone. It fails today on three files.
...
It is walked on the OTA stack after 0133 T4 (the mail half) and with 0131 T1 and T2 in place. The
two accounts are fresh, and neither is on the owner's domains. A uses a Dutch browser and B an
English one. It follows on from 0133 T4's step 4 (joined as owner):

1. The dashboard is in the browser's language, and the page's `lang` follows the switch (fixed in
   #1137, pending merge). The alpha note shows (0131 T1).
...
6. The owner issues a grant link for a Google source that names A's account.
   - Opened while signed in to Google as B, it is refused, and nothing is stored (`grant.ts`:29-34).
...
For each step, record pass or fail, the date, the deployed commit and the language seen. Never an
...
**(c) The real-API journey (🅿️ Parked, trigger: 0132 T8).** The managed counterpart of
`selfhost-ui.e2e.test.ts` in the gate. It uses the smoke's real sign-in and runs request → grant →
wizard → first pass → view link. It needs a gate stack that is not the alpha's.
...
from `runs-on` (`e2e-o365.yml`:85). #1137 leaves this choice to the owner. Also check in the
...
**The rule (proposed).** The alpha's first deploy (0132 T6 step 1) is a commit C that meets both
of these:
...
**The order, with 0132.** 0132 T0's first step switches the gate off. So the count is earned before
that step, or the owner accepts fewer in writing (0131 T5's rule). With N = 5 and one scheduled run
a night, that takes at least five nights from the first green scheduled run. When #1137 merges, it
asks for one dispatched run first, for Node 24. That run is a precondition, not part of the count.

**During the alpha, with the gate off,** no count can build up. A deploy's evidence is CI green on
the commit plus 0132 T6's checks, written in 0132's deploy log.
...
2. **Counting (T14)** starts once #1137 has merged and its dispatched run is green. It runs in
   parallel with everything else.
3. **The owner's sittings, cheapest first:** T9, T5, T4, T2 and T3's Dropbox half. Any sitting that
   needs the demo Nextcloud, including T7's source, happens before 0132 T5 removes it (open
   question 3).
4. **T12's walk,** after 0133 T4.
5. **T14 is read, then 0132 T0.** The first invitation follows. Each later tester is granted once
   the sources they named have their proof, or have the label and nothing more, as open question 1
   settles.
...
1. **Does a tester who names a source wait for that source's proof?** This plan proposes waiting
   ...
   alternative is 0131 open question 3 (a): the label is enough for every source. Recommended:
   wait, because each of those proofs is one sitting, and the Microsoft calendar may need a rebuild
   rather than a fix.
...
3. **Where the proofs write to after 0132 T5.** 0132 T5 removes the demo Nextcloud and Stalwart.
   Where do the owner's proof targets live after that, and where does T7's source come from? One
   option is to run the sittings that need the demo Nextcloud before 0132 T5. The other is to bring
   up a Nextcloud the owner runs for themselves, which survives the reset.
...
6. **N, and dispatched runs.** Recommended: N = 5, the review's number, and scheduled runs only.
   0131 T5 and 0132 T1 read the same N.
