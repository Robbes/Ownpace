...
| [0020](./0020-managed-stack-productionization.md) | Productionizing the managed stack | ⬜ **Planned (2026-08-01 whole-repo review).** The security findings first: T1 — `auth.ts` trusts `tenantId`/`role` claims verbatim with no `tenant_member` lookup (anyone holding `JWT_SECRET` picks any tenant as owner); T2 — fail-closed secrets (`JWT_SECRET` defaults to `change-this-in-production`, five trigger secrets have hardcoded fallbacks, both env examples incomplete). Then moving the 0018 bring-up out of chat history: the TLS sidecar into compose, the managed runbook, `smoke-managed.sh`, scripted task env vars, a small hardening sweep (webhook sink, `managed-simple.yml`, last `/v3` import, uname preflight), and the owner decision on the polling scheduler's future. |
...
Mid-flight: **0018** (one apply smoke re-run from done — see its Status block). Blocked
deliberately: **0014** (Proton, on auth). Planned and not started: **0019/0020/0021** (the
2026-08-01 review's harvest) and 0015's Windows-only remainder.
