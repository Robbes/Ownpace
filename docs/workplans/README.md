...
| [0033](./0033-managed-screens-ledger-truth.md) | UX: managed screens tell the ledger's story | ⬜ **Planned 2026-08-09** — from the full UX review. The headline defect is load-bearing: the client's `MappingSchema` parses a lifecycle (`draft/completed/error`) the server never sends, so a mapping reaching `cutover` or `done` — the end every migration aims for — crashes the Mappings list and Dashboard; the "Completed"/"Errors" tiles count words that cannot occur. Plus three rule-9 maskings on the managed home screens (failed list read renders as an empty table; manual sync failure goes to console.error; wizard submit failure renders nothing) and live per-domain progress for the hub in both editions. Correctness first: this plan is sequenced ahead of the other three. |
| [0034](./0034-one-journey-per-edition.md) | UX: navigation & IA, one journey per edition | ⬜ **Planned 2026-08-09.** The appliance cannot click its way to its own per-mapping hub — the runs panel shipped 2026-08-09 into a page only reachable by typing a URL on selfhost; the appliance sidebar impersonates a login (avatar "U", user@example.com, a dead Sign out) on the edition with no accounts; per-mapping screens don't name their mapping in the chrome; the cutover order the nav encodes is stated nowhere; wrong-edition URLs render screens erroring against APIs that don't exist. Five bounded tasks, no IA merge — the flat-nav vs hub split is a real edition difference and stays. |
| [0035](./0035-one-vocabulary-two-languages.md) | UX: terminology & i18n completion | ⬜ **Planned 2026-08-09.** Four dialects of "state" on screen (Succeeded vs Completed vs raw `in_progress` vs lifecycle words) → one dictionary-owned vocabulary + one StateChip, with a WRITTEN rule for which words are server-vocabulary and stay verbatim (PASS/FAIL, evidence, refusals — the prose boundary is the constraint, not the victim). Plus the unrecorded EN-only screens beyond the parked Dashboard/Mappings debt: the QueueScreen scaffold itself, DiscoveryCounts' table headers (on the appliance's flagship screen), Login, the whole CreateMapping wizard, Billing — and a guard test so the class ends. Carries the one owner question: fold Dashboard/Mappings localization into 0033's rework (closing 0024-T5 at near-zero marginal cost), or keep parked. |
| [0036](./0036-as-of-and-aftermath.md) | UX: status honesty polish (as-of & aftermath) | ⬜ **Planned 2026-08-09.** The Windows weekend's most expensive confusions were all one species — a number with no statement of what it counted or when. The worst sites were fixed (#348, #355); this plan makes the discipline uniform: as-of timestamps on the queues (they refetch on focus, invisibly)/Verify/LiveProgress, aftermath parity across queues (Moves and Failures lack the "Already decided" half Deletions has), RunsPanel stating its 20/25 truncation bounds, a Failures→run-history link, and one honest sentence on what retry costs (a full re-list next pass). |
...
4. **The UX review's four plans (0033–0036), in that order** — 0033 first
   because it is correctness wearing UX clothes: the managed screens crash on
   the lifecycle's own happy ending and mask failed reads as empty states.
   0034 (navigation) unblocks 0036 T3. 0035 carries the one owner question
   (fold the parked Dashboard/Mappings localization into 0033's rework).
