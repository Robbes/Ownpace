...
| [0024](./0024-bilingual-ui.md) | The bilingual UI (ADR-0013, kept) | 🟡 **T1 built (PR pending); T2 screens, T3 dates/notifications, T4 server-prose boundary open.** Owner decision 2026-08-02 (0021 T5): keep. Hand-rolled typed EN/NL dictionary (no i18n framework — two locales with compile-time key parity don't need one), LocaleProvider (persisted, browser-detected), localized chrome + switcher, and the destructive-path warning in Dutch beside its English source in `@openmig/shared`. Refusal prose stays verbatim (rule 2). |
