...
| [0018](./0018-trigger-task-deployment.md) | Deploying the Trigger.dev tasks | ⬜ **Planned, not started.** The one thing between 0017's designed 502s and the real managed verify/apply loop: the repo has seven `schemaTask` definitions and no `trigger.config.ts`, no deploy step, no registry (trigger-api points at `localhost:5000`, nothing listens), and three disagreeing env namings for the SDK's credentials — pinned down by the 2026-07-31 live smoke, whose exact curls become T5's acceptance evidence. Scoped to making verify/apply real against the self-hosted instance; moving syncs off the polling `managed-scheduler` is an explicit non-goal. |
