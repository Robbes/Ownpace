...
| [0016](./0016-pglite-adoption.md) | Adopting PGlite (the whole-workspace driver switch) | ✅ **Done, except the e2e run.** The blocker was **not real**: `pnpm add` relinks incrementally, and a clean `--frozen-lockfile` install resolves exactly one `drizzle-orm`. `pgliteDriver()` serialises `acquire()`, and `SELFHOST_PERSISTENCE=pglite` starts, migrates and serves the operating surface with no `DATABASE_URL`, no container and no `initdb`. Concurrency measured (P5): serialisation costs nothing at width 8 — flat 3.6–3.9 ms/item, and the T0 spike's ~1700 rows/s is corrected to ~270 items/s on the real hot path. **P4 found a real hole**: RLS was inert on BOTH appliance backends, because it connects as the owner or as `postgres` and Postgres exempts both — fixed with `LedgerDriver.role`, proved on each backend through the production wiring. Open: **P7**, the e2e run on `compose.pglite.yml` (needs a Docker runner).
| [0017](./0017-managed-apply-and-verify.md) | `apply` and `verify` in the managed edition | 🟡 **Shape decided (T0), nothing built.** The last gap ADR-0026 left open on purpose: both operations touch the target, and in the managed edition target I/O belongs to the worker behind Trigger.dev — not to a request thread holding connector credentials for minutes. Today `Verify.tsx` calls `GET /verify` with a 15-minute HTTP timeout, which works on the appliance and does not exist on managed; and `…/deletions/{hash}/apply`, the one destructive route in the product (ADR-0024), is appliance-only. **Decided:** start + poll — `POST` begins, `GET` reports `never-run`/`running`/`done`/`failed` — with both editions implementing both verbs, and the appliance's synchronous `GET /verify` kept for one release so the e2e gates keep working. `apply` is NOT symmetric: seven of its eight refusal reasons are decided against the ledger, so the managed route evaluates those synchronously and enqueues only once removal is permitted. Also carries a schema fix: `verification.status` allows three values where the contract has five, and `SKIPPED`/`NOT_VERIFIABLE` are exactly the two the UI refuses to soften. |
