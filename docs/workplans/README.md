...
| [0015](./0015-native-windows-installer.md) | Native Windows installer (no Docker, no terminal) | 🔬 **T0 spike done — PASS.** The goal: one `.msi`/`.exe`, end users never touching bash, a Linux filesystem or Docker (today's Windows path is Docker Desktop + WSL2, which fails that outright). Deleting the shell-out engines left the runtime pure JavaScript, reducing the problem to **Postgres** — the only native dependency left (ADR-0023). `scripts/spike-pglite-windows.mjs` proves **PGlite** (Postgres 18.3 compiled to WASM, in-process, no service, no port) runs the REAL 2580-line baseline migration unmodified: 26 tables, 96 RLS policies, `app_user` role, transaction-local `set_config`, and **RLS genuinely enforcing** (tenant A sees 1 of 2 rows; cross-tenant INSERT refused). ~1700 rows/s ≈ 28s of ledger writes for a 48k-message mailbox; 3ms indexed lookups; survives restart. **ADR-0023 survives untouched** — same SQL, same migrations, no SQLite fork. Two caveats recorded: `row_security` defaults to `off` (fails loud, not silent — but must be set explicitly), and `pgcrypto` needs the contrib import. **Was blocked on the UI**, deliberately: an installer for an appliance whose operating surface is `curl` is useless to the target user. That prerequisite is now **in progress** under [ADR-0026](../adr/0026-one-operating-ui-one-contract.md) — one React app served by both editions, against an operating contract extracted from the endpoints self-host already serves. The three decision-queue screens (deletions, moves, failures) exist and the contract is in `packages/shared`; still outstanding are the `verify`/`finish` screens, serving the bundle from the appliance, and the managed edition implementing the contract. |
| [0016](./0016-pglite-adoption.md) | Adopting PGlite (the whole-workspace driver switch) | ⏸️ **Blocked, deliberately parked.** The piece that cashes in 0015's T0 spike and T1 seam — until it lands, the appliance still requires a Postgres server, so neither of those delivers anything to a user on its own. **The blocker is a build break, not a design question:** installing `@electric-sql/pglite` makes pnpm resolve a second copy of `drizzle-orm` (it is an optional peer, so the store key changes) and the workspace then typechecks two nominally-distinct `SQL<unknown>` types against each other and fails outright. P0/P1 are to reproduce and resolve that — overrides first, `.npmrc` dedupe second, whole-workspace atomic change only as a last resort. Everything after it is small: `pgliteDriver()` is two methods, and the hard part (serialising `acquire()`, because one connection means two concurrent `withTenant` calls would interleave transactions and leak `app.current_tenant` across tenants) is already permitted by the seam and already tested against a fake driver. Then run the RLS integration suite against it — "96 policies created" is not "96 policies enforcing", and an in-process WASM database runs as owner, which Postgres exempts from RLS. Carries the two open questions 0015 could not settle: whether managed stays on server Postgres (so two backends ship and both need RLS coverage), and what serialisation costs on a real corpus rather than 5,000 synthetic inserts. |
