...
| [0031](./0031-jmap-full-target.md) | JMAP as a full target (calendars, contacts, files) | ⬜ **Planned (owner decision 2026-08-05, 0026 T3 row 18: BUILD).** ADR-0018's "mail leads" deferral expired when DAV was proven; the standing recommendation was to retract the promise and declare JMAP mail-only, and the owner overruled it — **JMAP is judged more future-proof and is therefore preferred**. Not a capability gap: Stalwart already serves these domains over DAV. What it buys is one protocol per target. **T0 is a spike that can end the plan**, and nothing starts until it answers: do our natural keys survive a DAV↔JMAP switch? Calendars carry a fresh scar (a recurring series and its modified occurrences share a UID; `RECURRENCE-ID` was added to the key on 2026-08-04 after it silently lost occurrences), and files carry path normalisation, which has produced four silent-mismatch bugs already. If the keys cannot be made to agree, a mapping is not switchable and that comes back to the owner rather than being absorbed. |
| [0032](./0032-imapflow-migration.md) | Migrate off `imap-simple` to `imapflow` | 🟡 **T0–T3a done 2026-08-06; the e2e acceptance gate is GREEN on the flipped path, so only T3b (dropping the dependency) is open.** Originally planned (owner decision 2026-08-05, 0026 T3 row 21: MIGRATE).** ADR-0022 said to move and called it not urgent; no decision was ever recorded. Scoped rather than started because the row understated it — 1430 lines across the two connectors plus six test files, and `imapflow`'s API differs fundamentally. **T0 is a parity harness running BOTH clients against the same fixtures**, before either connector is touched: the risk is `internetMessageId`, which `naturalKeyForItem()` hashes, and a differently-normalising client re-copies every message while every write succeeds. Read path before write path, because the read can be wrong loudly and the write can be wrong quietly. The nightly e2e is the acceptance test, not the unit suite. |
