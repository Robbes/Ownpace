...
| [0015](./0015-native-windows-installer.md) | Native Windows installer (no Docker, no terminal) | 🔬 **T0/T1 done; T2 decided; T3 half-built.** The goal: one `.msi`/`.exe`, end users never touching bash, a Linux filesystem or Docker. Deleting the shell-out engines left the runtime pure JavaScript, reducing the problem to **Postgres** — the last native dependency (ADR-0023). **That is now removed**: the appliance runs on PGlite with no `DATABASE_URL` (see 0016). The UI prerequisite is met — [ADR-0026](../adr/0026-one-operating-ui-one-contract.md), one React app served by both editions, the confirm page folded into it and the old HTML deleted. **T2 decided in [ADR-0027](../adr/0027-windows-packaging-shell.md)**: Windows Service + Start-menu shortcut, no native shell. **T3**: `pnpm package:appliance` stages a 27.6 MB relocatable payload that boots detached from the repository, tested as a real child process — on Linux, which proves it is relocatable and proves nothing about Windows. The MSI, service registration and shortcut are Windows-only and not done, but as of 2026-08-06 the Windows work is **prepared**: **[docs/windows-appliance-runbook.md](../windows-appliance-runbook.md)** plus `scripts/windows/`, ordered by what is unknown rather than by what an installer needs, because nobody has ever run the appliance on Windows at all. Two defects were found by reading and are recorded there: `start.mjs` defaults its writable state to inside the payload, which is `C:\Program Files\` once installed and unwritable by a service account, and the UI port is 8080 in the code and payload but 8081 in the compose deployments and ADR-0027's shortcut. Open: T4 code signing (a purchasing decision), and whether the payload ships its own Node runtime.
