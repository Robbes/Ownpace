...
| [0022](./0022-syncs-on-trigger-tasks.md) | Syncs move onto Trigger.dev tasks (retiring the poller) | 🟡 **T0 decided, T1+T2 built (2026-08-01, PR open); T3 live cutover pending.** The 0020 T8 owner decision executed: one declarative `managed-sync-tick` (every minute, mapping crons evaluated against the DB — no per-mapping schedule rows to reconcile) triggers `run-delta-sync` with explicit enabled domains and per-mapping concurrency 1. Staged rollout: the poller keeps running until the tick has live evidence (T3), only then is it removed (T4). |
...
Mid-flight: **0022** (the sync-tick cutover — T1/T2 built, T3 live evidence pending; the poller stays as the rollback lever until then). **0018 and 0020 both closed 2026-08-01** with live evidence for every task.
