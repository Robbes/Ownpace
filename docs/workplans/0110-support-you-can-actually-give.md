...
**The fix is not more matching, and could not have been.** A source refusal and a target one read IDENTICALLY — a 403 is a 403, `Forbidden` is `Forbidden` — so no wording distinguishes them and matching harder would only have invented a signal. `failed_side`, added the week before for the connections page, already held the answer: `sided()` tags what `fetchRaw` throws as `source` and what `upsert` throws as `target`, at the closure, and `markFailed` receives that tag and the message in the SAME call. The whole change at the seam is `classifyFailure(error)` → `classifyFailure(error, side)`. The two columns are therefore written by one statement from one call and cannot disagree, which is the thing a second field saying "where" usually gets wrong.

`source_refused` is the answer when the pass named the source. **An unsided refusal still answers `target_refused`**, deliberately: `failed_side` is NULL for a pass that could not tell and for every row written before it existed, and re-reading those as source refusals would rewrite history on no evidence — a write is also where the overwhelming majority of refusals happen.

`format_refused` is the owner's own addition the same day: *"it also makes sense a target might refuse certain fileformats/types and we need to be transparrant about that."* A destination that will not store a `.svg` is not a destination that is full, and "free up space" is the same wrong remedy one step along. It matters most for Google exports, which arrive as `.docx`, `.svg` and `.pdf` — files the source never had. `415` anchors it (RFC 9110 §15.5.16, specified); the filename phrases are Nextcloud's published wording and are marked in the code as **not observed**, the same distinction `drive-refusal.ts` keeps. A bare 403 carrying a blocked extension is deliberately NOT matched — Sabre says only `Forbidden`, and guessing "format" from that would mislabel every permission problem.

**NO MIGRATION WAS NEEDED FOR THE VOCABULARY, which is migration 0033's design paying off.** That migration chose `text` with no CHECK and said why: *"the application owns the vocabulary."* This is the first revision and it took no lock — 0048 exists only to correct a COMMENT that enumerated the six. **What the exhaustive types caught, before a line of UI was touched:** both i18n maps (`Record<FailureCategory, StringKey>`) failed to compile, the OpenAPI enum guard reded at two sites, the copy budget refused two over-long `view.*` sentences, and the backoff complement named the two new ones — correctly, since both need a person. **What they did not catch, and a test now does:** `LiveProgress.unit.test.tsx` had hand-copied the key map as a literal of six, so its "every category has a sentence" loop was checking the six it knew about rather than the ones the product has, and `VIEW_FAILURE_KEY` had no string coverage at all. Both now walk the maps themselves, in both languages, including a check that a Dutch key is not the English sentence pasted across.

Four mutations killed, the first being the whole fix in one character: dropping `side` at the seam (the store test reds; every classifier test stays green, which is again why that file exists), removing the source reading from the general refusal rule, letting a source refusal keep the format reading, and relaxing `side === 'source'` to a truthiness test — which reads `'target'` as source and turns five assertions red. |
