...
- the whole `vCard` object is gone — escape hatch, `X-OPENMIG-PROBE` and the
  `convertedProperties` provenance map together;
- the single `ADR` became **two** addresses: `k1` holding nothing but
  `coordinates`, and a new `k1-2` holding the actual street address.

Every write returned success. This is the same shape as the calendar finding
and as the four vacuous passes fixed this week, which is why it gets rungs
instead of a workaround.

### T2.0a — the two rungs that diagnose it (built 2026-08-05, awaiting a run)

**Rung A: was `vCard` dropped, or merely not returned?** Re-reads the stored
card asking for `vCard` **by name**. A property explicitly requested and still
absent was not stored — which would mean every X- property is lost on the JMAP
path while the DAV path keeps it verbatim, making JMAP contacts strictly worse
than what this product already ships. That is an owner decision, not a bug fix.
Present, and nothing was lost; `get` simply did not volunteer it.
...
**T2.1 is still not written.** Route being settled is what unblocks the
connector's *shape*; these two rungs decide what the connector is allowed to
promise, and one of the three Rung B outcomes goes back to the owner rather
than into code.
