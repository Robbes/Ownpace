...
| T4 Surface + manifest truth | 🟡 **The CONTACTS slice done 2026-08-05, the FILES slice 2026-08-06; only the picker still waits, and only on T1** | **Files slice 2026-08-06:** the scope manifest's Files row now says the domain travels over WebDAV *or* JMAP, and says the thing that differs from the contacts row — there is NO narrowing here. A JMAP file node carries its own byte count and a handle to its content, so verification checks counts, total bytes and content checksums on both paths, which is why `JmapFileTarget` implements `contentHashFor` and `JmapContactTarget` deliberately does not. Version bumped to `2026-08-06`. **T4's PROBE built 2026-08-06 — the picker's missing prerequisite, not the picker.** `packages/connectors/src/jmap-capabilities.ts` — `probeJmapCapabilities()` answers, in one request, which domains a given server can carry. It exists because the remaining T4 item cannot be built without being able to ask, and it has value on its own: `TargetConfig` has allowed `type: 'jmap'` for contacts and files since those connectors landed, and a mapping naming it against a server that does not serve the URN currently finds out at WRITE time, mid-migration, as an opaque method error. **It keeps three things apart that a one-liner would conflate, and each is a real case rather than a hypothetical.** (1) *Advertised ≠ writable.* Every JMAP writer here resolves its account from `primaryAccounts[urn]` and refuses rather than guessing — writing a customer's contacts into somebody else's account being the worst thing those files could do — so a capability with no account behind it is a URN this product cannot write to. (2) *The server speaking it ≠ this product carrying it.* **Stalwart really does advertise `urn:ietf:params:jmap:calendars`, and T1 is really parked**, so a picker gating on the URN alone would offer a target that writes a recurring series as a single event plus orphaned overrides, silently. `serverSpeaks` and `supportedByThisProduct` are separate fields; `usable` is the conjunction. (3) *A failed probe ≠ a negative answer.* A session that cannot be loaded throws with the cause attached rather than returning a report full of `false`, which a picker would render as "this server speaks no JMAP" for what is actually a typo'd password (hard rule 9). Contacts additionally require `urn:ietf:params:jmap:contacts:parse`, because `JmapContactTarget` is built on route (2) — the server converts the vCard — and cannot work as built without it. **10 unit tests, mutation-verified four ways**: gating on the capability alone, ignoring the extra URNs, treating `serverSpeaks` as `usable`, and swallowing a failed session load each fail tests. **`jmap-capabilities.integration.test.ts` deliberately does not re-assert the URN list** — it calls `JmapContactTarget.connect()` and `JmapFileTarget.connect()`, which resolve their accounts from the same `primaryAccounts` entries, so the probe is checked against the connectors it describes rather than against a constant. That guard exists because the spike once checked `urn:ietf:params:jmap:blob` for files, reasoned about it correctly, and would have reported files unsupported while the server advertised `filenode` all along — perfect logic over the wrong constant is exactly how a probe stays self-consistently wrong forever. **THE INTEGRATION TEST FAILED FIRST TIME, on the one case that mattered most — and the bug was a dependency behaving as it does rather than as assumed.** Three of its four cases passed: the server's capabilities read correctly and both connectors connected. The fourth, *a wrong credential must throw rather than report nothing usable*, RESOLVED with `advertised: []`. Cause: the probe called `JamClient.loadSession`, whose entire body is `fetch(url, {headers}).then(r => r.json())` — **it never checks `response.ok`**. A 401 carrying a JSON body parses happily, so the helper resolves with the ERROR DOCUMENT: no `capabilities`, no `primaryAccounts`. Read through the probe, that document said *the server advertises nothing* — the exact conflation the probe was built to prevent, arriving by a route the `try/catch` could never see because the library does not throw. A wrong password would have been presented to a picker as "this server speaks no JMAP", sending an operator off to configure a different protocol. Fixed by fetching the session directly and checking the status; the unit tests now stub the TRANSPORT rather than the library, because mocking `jmap-jam` would reinstate the very assumption that proved false. Mutation-verified: removing the `response.ok` check fails 2 tests. **That follow-up is now DONE (2026-08-06).** `packages/connectors/src/jmap-session.ts` — `loadJmapSession()`, the one place any of this reads a session document, and all four call sites use it: the probe plus `jmap-target.ts`, `jmap-contact-target.ts` and `jmap-file-target.ts`. Shared rather than copied because a second implementation of *check the status* is a second place for it to go missing. The writers were never at risk of a silent wrong answer — a session with no accounts makes their `connect()` refuse rather than guess — but they reported a REJECTED CREDENTIAL as a failure to resolve an ACCOUNT, which sends an operator to audit provisioning on a server that only ever said *wrong password*. The message now leads with what to go and fix: 401 names the credential, 403 says the credential was accepted and not permitted (two different places to look), 404 asks whether this is a JMAP server at that path, and an unfamiliar status gets **no invented interpretation** — reported as itself, since guessing a cause is the same class of mistake in the other direction. The probe keeps its own *UNKNOWN — not "none"* framing by wrapping, because only that caller has reason to say what the failure does not mean. **The bigger change is in the TESTS.** `jmap-file-target.unit.test.ts` and `jmap-contact-target.unit.test.ts` both carried `vi.mock('jmap-jam')` stubs asserting something untrue — that session loading either succeeds or throws — so no test built on them could ever have seen this bug; both now serve the session through the fake transport and run the real loader. 16 new unit tests, **mutation-verified eight ways**: reinstating the original bug (dropping the `response.ok` check) fails 11 tests across three files, and collapsing 401 into 403, dropping `cause`, removing body truncation, dropping the `Authorization` header, letting a body-read failure escape instead of reporting the status, returning `{}` for a non-JSON 200, and unwrapping the probe's framing each fail tests. **Still open, and now an OWNER DECISION rather than a blocked task: the picker itself.** `apps/web/src/pages/CreateMapping.tsx` offers ONE `targetType` for the whole mapping (`'jmap' | 'imap' | 'caldav' | 'carddav' | 'webdav'`), so "JMAP per domain" is a restructure of that screen rather than a filter on it — plus bilingual strings and the appliance's own equivalent (hard rule 5). The probe makes it buildable; what shape it should take is a product question. Contacts slice, 2026-08-05: | T4 was scoped as one task behind T1, but T1 is parked and T2 shipped — so the part of it that had become UNTRUE was done now rather than held behind a domain that may not arrive. **Scope manifest** (§11.2, no silent omissions): Contacts now says it travels over CardDAV *or* JMAP and names the one difference that matters before cutover — on the JMAP path the target exposes no route back to the original vCard, so content-checksum sampling does not run and verification checks counts and presence instead. Version bumped. **ADR-0018** gets its dated update: "mail leads and cal/contacts/files follow" now has a first follower, and the three domains did not move together — contacts shipped, calendars are parked on a server gap, files are buildable and unstarted. **Two stale truths corrected**, both saying CalDAV/CardDAV omit `contentHashFor`: that was #143's position and it was REVERSED, so both DAV writers hash canonically now and the accurate present-day example is `JmapContactTarget`. One was a present-tense comment in `verification-implementations.ts`; the other was the NAME of the test that pins the property — and a test named after a case that no longer exists is how somebody deletes it as obsolete. **Nothing was built for the narrowing itself**: `verification.ts` already raises `CHECKSUM_UNAVAILABLE_*` as a WARNING plus a recommendation, so the JMAP contacts path degrades honestly through machinery that already existed. **Still open: the target picker offering JMAP per domain only where the server speaks it**, which needs T1 and T3 to know what it is offering. |
