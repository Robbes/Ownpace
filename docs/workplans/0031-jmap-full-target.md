...
- **Not in the nightly e2e.** ~~The integration test has to be run by hand.~~
  **Corrected 2026-08-05: it runs in CI's `integration-tests` job on every
  push**, which is a stronger and more frequent gate than the nightly. What is
  genuinely missing is different and narrower: the e2e drives a full
  `runDomainSync` — ledger rows, cursors, verification, restart-resume — and its
  contacts domain is CardDAV→CardDAV
  (`test/e2e/fixtures/selfhost-restart-resume.mapping.json`). A JMAP variant
  needs a second mapping fixture pointing `contacts.target` at
  `type: 'jmap'`, plus the appliance container being able to reach Stalwart,
  which it already can — mail uses it. **The connector is exercised; the LOOP
  over it is not.**
