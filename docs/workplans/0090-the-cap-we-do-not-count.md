...
| T4 Refuse before the lockout, not after | ✅ **Done 2026-08-26** | `runDomainSync` takes an optional `downloadMeter` (the SAME instance the connector spends, so the state the gate reads is the state the fetches moved) and reads it at pass start and before every fetch — deliberately after the ledger fast-path, since a skip downloads nothing. At zero remaining the pass STOPS AND SAYS SO: the T4 sentence in the log (the limit, today's usage, when it resets, continues by itself; named from OUR meter, never from an unobserved Google response) and a `budgetPause` record on the result carried through `ReconcileResult`. Everything about HOW it stops is pinned by tests: a pause is NOT a failure (no ledger row, no retry counter, no failure-queue entry — the items were never looked at), the paused folder KEEPS its cursor (the 0015 lesson: a ceiling nobody named looks exactly like a bug; and an advanced cursor would retire tomorrow's items), later folders are never listed (never retry into a quota that counts down to a window — the JMAP lesson), and a pass that begins spent lists nothing at all. Overshoot bounded by `concurrency` in-flight bodies, documented at the gate. 7 unit tests; proofs by breaking (pause-as-failure → 2 red, cursor-advance-on-pause → 1 red). Still open, deliberately: carrying the sentence into the summary mail rides with the digest (0104 T3's surface), not this loop. **2026-09-08 — the pause reached a reader at last.** `budgetPause` was produced by the loop, carried through `ReconcileResult`, and consumed by NOTHING: `run-delta-sync` marked a paused domain `completed` and logged "N created, M skipped" as though the mailbox had finished. It now marks a paused domain neither completed nor failed — it stays `in_progress`, which is what it is across passes — and writes an `info` run event naming which ceiling was reached, how much of it was spent and when it resets. Found while building the pass deadline, which stops a pass the same way and would have inherited the same silence. **2026-09-08, second half — the pause reached the CUSTOMER.** A run event is where an engineer looks; the person whose mailbox has stopped moving is looking at the migration's own page, and it said nothing for the hours a ceiling lasts. Ledger migration 0041 adds `migration_status.paused_reason` (jsonb, read back through `isPauseReason`, cleared by whatever writes the row next), `markPaused` writes it, and BOTH dispatchers now call it — the appliance's `runOneDomain` had the identical silence and would have kept it, which is the edition split hard rule 5 forbids, in the edition where the owner has nobody to phone. On screen: whose limit, and when copying continues, with the reassurance folded under "Why?" (#124's precedent — a hover fails on touch, keyboard and screen reader). The byte figures stay on the run log: 2 500 000 001 of 2 500 000 000 is evidence for an engineer and unverifiable noise for the person whose mail it is. |
| T5 Say it in the price | ✅ **Done 2026-08-26** — built into 0088 T3's calculator from birth | `gmailMailDays` in `site/calculator.mjs` (`GMAIL_IMAP_GB_PER_DAY = 2.5`, the T1-verified number, decimal reading so an arithmetic doubt errs toward MORE days): when the source is Google and mail is ticked, the page says *"Google caps Gmail IMAP downloads at 2.5 GB per account per day, so N GB of mail needs at least M days — that minimum comes from Google's published ceiling, not from a bandwidth guess"*, flags a chosen duration shorter than the minimum (*"the mail sets the pace here"*), and carries the counter-argument in the same sentence: it is exactly why Ownpace syncs continuously and cuts over when you are ready. Unit-tested (8 GB → 4 days, the SME 160 GB → 64 days — past a 1-month choice), and the machinery's presence on both rendered locales is grep-guarded. |
