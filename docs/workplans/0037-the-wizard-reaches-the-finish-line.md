...
| T4 Choices that cannot work are refused, not stored | ✅ Done 2026-08-10 | Shared matrix `TARGET_TYPE_DOMAINS` + `targetDomainRefusal` (packages/shared/src/target-domains.ts) mirrors the engines (jmap = email/contact/file — no calendar, 0031 T1 parked); the wizard disables unreachable data types (selected-but-incompatible stays deselectable) and the create API refuses the combination via `CreateMappingSchema.superRefine`, naming both sides, with the sentence in the 400 `message` where `serverMessage()` renders it. Cron: conservative shared validator `describeCronScheduleProblem` (subset of croner's grammar; containment pinned by `apps/worker/src/cron-schedule-parity.unit.test.ts` against the tick's own croner version); wizard validates live and echoes the next three firings via the same pinned croner; server refuses garbage naming the silent 15-minute fallback it prevents. Contract tests: `create-coherence.unit.test.ts` (schema) + wire-level 400s in `create-mapping.integration.test.ts` (whose own payload was the jmap+calendar incoherence — now email+contact). Appliance create path needs no twin: an unsupported pairing throws in the per-domain factories and an invalid cron throws in croner at startup — both already loud. **2026-09-08 — asked again when the pass runs, for all five domains.** Every gate above runs ONCE, at creation, and the ground moves under them: `TARGET_TYPE_DOMAINS` gained `task` (0113) and `nextcloud` (2026-09-07), and a grant can be revoked long after a mapping is made — while `scope_selection` is written once and never updated, so a stale tick has nothing re-asking about it. #858 gave MAIL the question in place after the owner's preflight read `Unsupported target type: undefined`; the other four had none. `refuseDomainTheTargetCannotCarry` (`packages/orchestration/src/pass-domain-refusal.ts`) now answers it for all five, from the same table and in the same sentence, called from both deps seams — `buildDepsFromMapping` (mail) and `buildDomainDepsFromMapping` (the rest). A kind the table has never heard of is deliberately left alone: connection kinds outnumber wizard target types, and refusing every unrecognised one would break working migrations to make a point about a table this function does not own. The SOURCE half is deliberately NOT asked here — it needs the account's measured qualification rather than a static table, and `measuredNoRefusal` already reads that record at creation; a second differently-shaped reading in the pass path is how two answers to one question start disagreeing. Guard: `a-domain-whose-pass-never-asked.unit.test.ts` (18, derived from the table), proved by breaking both the seam and the refusal. |
...
| T6 Per-source-type credentials (owner scoping) | ✅ Done 2026-08-10 (owner answered: real OAuth via the per-customer app registration) | oauth2/graph collect the ADR-0006 row-14 model: tenant ID + client ID on the source step, client secret + mailbox UPN on the credentials step (`wizard.source.appRegistration` explains the model and points at docs/o365-setup.md; the interim `credsOnly` confession is gone). `CreateMappingSchema` demands the registration by name per source type; the POST path encrypts `{username, tenantId, clientId, clientSecret}` — exactly the keys `build-deps-from-mapping.ts` reads — and writes ENGINE-shaped connection configs (`graph-mail` with tenant+mailbox; `imap-oauth2` with host/user; jmap/imap-dav targets typed too). That last part also fixes a discovered break: create stored `{host, port, useSsl}` with no `type`/`user`, so the worker refused every wizard-created mapping at build time ("got: undefined") — only the dev seed wrote a buildable config. The managed IMAP factory now mints XOAUTH2 tokens from an app registration alone (client-credentials, `.default` scope; same MsalTokenProvider as Graph). Tests: schema refusals (create-coherence), wire-level 400 + stored-shape 201 (create-mapping.integration), worker factory (build-deps-from-mapping.unit), wizard collection + submit payload (CreateMapping.unit). |
