...
`applyMappingStatusChange`) moved the paths but raised no peak, and a comment said the next read
of the tier would true it up. Nothing in production reads it that way: the true-up is
`currentTier`, for the invoice, and the screens read `observedTier`, which writes nothing. So a
rollback's slots were in no month's peak unless something else raised the mark. Now the door
takes `onSlotsTaken`, run in the change's own transaction when its paths took slots, and both
callers pass `raiseThePeakWhereThereIsOne` (apps/worker). It raises the mark where the database
keeps one, and on a self-hosted database, which keeps none, writes nothing and never loads the
managed package. **Evidence:** the ledger door on PGlite with the ledger chain only (7: once and
inside the change, taken back whole when the peak fails, passed on by the port, nothing without a
hook, and no call for a cutover, a restated status or a migration with no paths); the helper on
a managed and a self-hosted database (2); and a guard that every caller of the door outside the
ledger passes it (1).
...
**T1c decided, 2026-09-24:** the owner split the cutover per data type (*"we need to split up
cutover, since someone might want to keep syncing some kinds, like keeps files running, while
email cutover/stops"*). The announcement question resolves per data type too: each data type's
shares are announced once, at that data type's cutover (0128 D8). The work continues as
workplan 0128 T5, where the machine, the grace period and rollback are designed per data type.
