...
|---|---|---|
| T1 solution-architecture.md v1.2 | ✅ **Done — merged (PR #228, CI green)** | Version bumped to 1.2 with a dated change note. The engine-name sweep executed: every imapsync/vdirsyncer/rclone row in §7.3/§8 (mermaid)/§9/§10/§11/§12/§13/§14/§21 now names our own TypeScript connectors, and the §6 banner reads as history instead of a decoder ring (the two remaining mentions are the banner itself and §9.4's *rejected* rclone Proton backend). §3.2/§18 corrected to ADR-0011's real position (self-hosted targets permitted, user-operated). §11.1's apply paragraph corrected — the IMAP/DAV mail target DOES implement `TargetRemover` (verified: `ImapDavMailTarget implements … TargetRemover`, `removeItem` reports `deleted`). PGlite named in §7.3 State row + §22.1 (ADR-0028). §23 got an honest update note: bilingual UI is an unbuilt promise pending the T5 owner decision (header + §3.8 marked too). §24 ADR index extended 0024–0028. §25's Windows row rewritten per ADR-0027/workplan 0015. Bonus truth fix: §13.2's "reuse an external one-shot JMAP import utility" plan replaced with the built reality (our own JMAP writer does bulk + incremental). |
| T2 The runbooks tell the truth | ✅ **Done — merged (PR #229, CI green incl. the 6 new cutover-CLI tests)** | `cutover-runbook.md` REWRITTEN against the code: `tsx` invocations (ts-node is not installed), `--yes` on every state-changing command, the verify-only DNS truth ("execute does not change DNS; the MX switch is YOUR manual step"), the real `verify` legs (MX blocking, SPF/DKIM/DMARC/autodiscover warnings, §20 data gate blocking, auto-advance to READY_FOR_CUTOVER), GRACE_PERIOD in the state diagram, the `runbook` subcommand documented. **Grounding it found a real bug**: `execute` transitioned CUTOVER_IN_PROGRESS→COMPLETED, an edge VALID_TRANSITIONS does not have — the happy path threw "Invalid transition" AFTER the operator switched DNS. Fixed: `execute` lands in GRACE_PERIOD, new `--yes`-gated `complete` subcommand closes GRACE_PERIOD→COMPLETED; 6 new unit tests (21/21 in the suite); CLI help texts moved to `tsx` too. `rollback-mechanisms.md`: tsx + `--yes` in the examples. `operator-runbook.md` appliance half: scope banner declaring the ledger-semantics sections both-edition (managed = per-mapping API/screens), all appliance URLs unified on the compose port 8081 (source-run default 8080 stated once), stale worker-container references fixed (logs/recreate/dev-from-source/two-DB-roles), quickstart link de-futured. `selfhost-quickstart.md`: PGlite variant documented (compose.pglite.yml, SELFHOST_PERSISTENCE=pglite, one container, no pg_dump — stop-and-copy backup recipe), footprint note un-lied (ADR-0028), port truth (compose default 8081, all examples unified), `GET /` → `/ui/confirm` redirect noted. |
| T3 Reference docs catch up with the code | ✅ **Done — merged (PR #230, CI green)** | `rls-guide.md` REWRITTEN against the code: the three-part enforcement model (FORCE on every RLS table — 22 in the baseline + 2 stragglers in migration 0002 + verification_run/apply_receipt in 0003/0004 = **26 tables, all FORCEd**; the non-owner `app_user` role "without it, RLS does nothing"; `LedgerDriver.role` + `withTenant`'s BEGIN → SET LOCAL ROLE → transaction-local `set_config` order), the real table list, the 0020 T1 membership gate, and the REAL suites (`force-rls`, `rls-in-force` ×2, `pglite-driver`, `rls.integration`, the seeded API route suites) — `pnpm test:rls` never existed. `testing.md`: the `GenericSyncEngine` property section replaced with the real `runDomainSync`/`dav-sync.integration` pattern; "WebDAV files remains deferred" corrected (all four domains in the e2e gate since 2026-07-27); e2e dispatch inputs documented (`seed_count`, `persistence: postgres\|pglite`); `fixture-uuid-check` added to the CI map; NEW **untested-seams appendix verified against the tree 2026-08-02** (task wrappers, `build-deps-from-mapping`, `enabled-domains`, 10 web pages, 5 services/stores; Mollie webhook REMOVED from the list — its coverage exists, mocked-client only). `performance.md`: the P5 `bench:pglite` numbers recorded (flat 3.6–3.9 ms/item widths 1–16, ~270 items/s real, ledger ~7% of wall clock); stale test names fixed. `test-fixture-uuid-collision-audit.md`: REMEDIATED banner naming the shipped `fixture-uuid-check` guard. |
| T4 READMEs stop lying about env and ports | ✅ **Done — merged (PR #231, CI green)** | `apps/api/README.md` REWRITTEN: the real env contract (`APP_DATABASE_URL` + `SECRET_ENCRYPTION_KEY` + `DATABASE_URL` + `JWT_SECRET` required; `TRIGGER_API_URL`/`TRIGGER_SECRET_KEY` via `@openmig/scheduler` — `TRIGGER_DEV_API_KEY` never existed), no `pnpm migrate` (boot-time migrations), the deleted trigger-webhook route gone, role-from-row auth, the real route surface + pins, no Helm. `apps/worker/README.md` REWRITTEN: not a service — the eight tasks tabulated (tick/delta/full/discovery/verification/apply/cutover-prep/rollback), the in-runner `TRIGGER_API_URL_IN_NETWORK` gotcha, deploy-tasks/set-task-env/smoke, the cutover CLI incl. `complete`, honest rollback (no DNS restore), the trigger-client corrected to `packages/scheduler`, no `trigger.yml`, no webhooks. `apps/web/README.md`: port 3123, `react-router` v8 (not -dom), `build:selfhost` (`--base=/ui/`) documented, real env (`VITE_API_URL` optional default `/api`, `VITE_EDITION` set by the build), no-password-login truth, operating pages listed. `deploy/selfhost/README.md`: `compose.pglite.yml` + `compose.dev.yml` listed, single-container PGlite quickstart, port 8081. `packages/ledger/README.md`: migrations 0002–0004 named, the driver seam + `pglite-driver` + `LedgerDriver.role` section, `withTenantDb` → `withTenant`. Root `README.md`: unit tests need NO Docker, `dev.yml` has no Stalwart, `tsx` not `ts-node`, status paragraphs brought to post-0022 truth. |
| T5 Owner decisions on the broken promises | ✅ **Done — all 6 decided** | **ADR-0015 extra backup: RETRACT (owner, 2026-08-02)** — the opt-in extra-backup bullet retracted with the owner's rationale recorded (a second open-migrate instance/mapping to a second target is the same result through the existing idempotent engine); stack-DR and target-durability bullets stand; `backup_target` stays as reserved schema (no destructive migration; the Atlas lint guards any future drop); SAD §15/§24 + ledger README corrected.  **ADR-0013 bilingual UI: KEEP + BUILD (owner, 2026-08-02) → workplan 0024** — T1 built the same day (typed EN/NL dictionary with compile-time key parity, LocaleProvider + persisted browser-detected locale, localized chrome + WCAG-labelled switcher, APPLY_FLAG_WARNING_NL beside its EN source in shared; refusal prose deliberately verbatim per rule 2); T2–T4 defined there.  **ADR-0008 provisioner: RETRACT (owner, 2026-08-02)** — dated retraction note with a named revisit condition (hoster/reseller partnership, or a requested zero-touch onboarding feature); the one-line stub `packages/provisioner` DELETED (lockfile regenerated, ci.yml filter pruned, AGENTS.md + SAD §7.3/§24 corrected).  **ADR-0006 Graph-mail fallback: KEEP + BUILD (owner, 2026-08-02) → workplan 0023, ALL TASKS CLOSED the same day** (#233 connector, #234 wiring, #235 runtime fallback — all CI green); ADR-0006 carries the dated update note describing the complete build.  **ADR-0017 Atlas lint: KEEP + BUILD (owner, 2026-08-02) — built the same day**: `migration-lint` CI job (community Atlas binary, replays the whole migration dir against a disposable dockerized Postgres via `--dev-url docker://postgres/16/dev`, fails on destructive changes; `atlas.sum` generated in-job, not committed; runs when `packages/ledger/migrations/**` or the workflow changes). ADR-0017 carries the dated update note (incl. the ADR-0023 SQLite supersession).  **Worker `--config` CLI: KEEP (owner, 2026-08-02) — the last T5 item.** `apps/worker/src/index.ts` stays as a deliberately-kept dev tool (debug a connector / reproduce a support case for one mapping without booting an edition); it shares `runAllDomains` with both editions so it cannot drift silently, runs the ordinary non-destructive shadow pass, takes secrets from env only, and has no live caller by design. `apps/worker/README.md` documents it as such (its "fate pending" line replaced with the decision). T5 closed; only T6's SECURITY.md contact still needs the owner. |
| T6 Index, changelog, security contact | ✅ **Done — the plan is closed** | Mechanical half merged (#236): five ADR update notes (0002 shell-out dead; 0004 bytes-through-tasks with the privacy property restated; 0005→0024 forward link; 0016 squashed baseline path; 0019 SQLite→PGlite), the 2026-08-02 cycle CHANGELOG entry, and the docs index VERIFIED complete against `ls docs/*.md` (already fixed by the review PR — recorded as checked, not silently skipped). **SECURITY.md contact: GitHub Security Advisories (owner, 2026-08-02)** — the "(add contact)" placeholder replaced with the repo's "Report a vulnerability" link as the only reporting channel. ⚠️ One setting only the owner can flip, outside the repo: **enable private vulnerability reporting** (GitHub → Settings → Code security) or the advisories button rejects reporters. |
