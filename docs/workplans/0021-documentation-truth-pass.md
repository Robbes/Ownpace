...
| T6 Index, changelog, security contact | 🟡 **Mechanical half merged (#236)** | Five ADR update notes landed (0002 shell-out dead; 0004 bytes-through-tasks with the privacy property restated; 0005→0024 forward link; 0016 squashed baseline path; 0019 SQLite→PGlite), the 2026-08-02 cycle CHANGELOG entry written, and the docs index VERIFIED complete against `ls docs/*.md` (already fixed by the review PR — recorded as checked, not silently skipped). Open: the SECURITY.md contact — needs the owner (an email or a GitHub Security Advisories pointer). |
