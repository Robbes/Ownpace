...
| T5 the legal surface, which gates taking money | 🟡 **Drafted, not published** | **2026-08-20:** [`site/legal/privacy.md`](../../site/legal/privacy.md) and [`site/legal/terms.md`](../../site/legal/terms.md) written, plus [`site/legal/README.md`](../../site/legal/README.md) and a guard (`scripts/legal-docs.unit.test.ts`, 6 assertions, two mutation-verified). Written against what the code actually does — the ledger holds metadata not bodies, self-host sends us nothing, nothing is deleted at the source — rather than from a template. **Both carry `«PLACEHOLDER»` tokens and must not be published as they stand**; the README lists all twelve and the guard fails on an unlisted one. **Still missing and still the critical path: the DPA and the sub-processor list**, which the privacy policy links to and a business customer cannot lawfully be onboarded without. Brand assets landed with them (`site/brand/`, generated by `scripts/make-logo.py`) because Google's OAuth verification needs a ≥120×120 logo and a support address — see [`docs/google-oauth-verification.md`](../google-oauth-verification.md). **Still the real critical path, and it is not the pages**: the DPA (§17 makes the operator the processor and the tenant admin the controller, so it is not optional once a real customer's mail is involved), the sub-processor list, and the legal entity, VAT registration and invoice address that `pricing.ts` already knows how to raise — all twelve of which are the `«PLACEHOLDER»` rows in the README. **Widened 2026-08-28 by [workplan 0111](0111-an-invoice-that-is-a-document.md), because the market is consumer-primary:** (a) **the processor framing does not hold for households.** privacy.md §3 says the customer is controller for migration content and we are processor — sound for a business, but GDPR art. 2(2)(c) puts a natural person acting for purely personal or household purposes OUTSIDE the Regulation, and recital 18 keeps the PROVIDER inside it. There is then no controller for us to be a processor for, and we determine purposes and means anyway, so §3 needs a third branch — a private individual migrating their own or their family's mail, with Ownpace controller on Art. 6(1)(b). The README's summary line ("a processor for migration content") becomes true-for-business, incomplete-for-consumers. The **DPA stays a business instrument** — a family neither gets nor needs one, so the privacy policy must carry what the DPA would have said. And a child's mailbox holds mail from teachers, friends and other families who contracted with nobody: the parent's household exemption covers the parent and nothing for us. **Wants a lawyer, not this plan.** (b) **consumer contract law**: prices must be shown INCLUDING VAT (which makes 0088's calculator a required field, not a courtesy), the 14-day right of withdrawal needs explicit consent-to-begin plus acknowledgement that it is lost, and the current terms are written business-shaped. |
