...
Dropbox docs and guides, and a 0144 known-limitations entry; exporting Paper docs comes after. So
T5's table gains a 0150 row, and its range reads 0132 to 0150. §6 gives the work to M, as M8,
because it is connector work (0150 D7). Nothing else in this plan changes.
...
| 0150 Dropbox native formats (row added 2026-09-26, 0150 D6) | 0150's T5 and T6 (a): a Paper doc, and any entry Dropbox lists as a file but will not download, is refused by name with a stated category and parked on its first attempt. Until 0150 T3 lands, the refusal points to no setting. T9's lines: `docs/dropbox-setup.md` and the Dropbox guides say what the code then does, and so does a 0144 known-limitations entry. Exporting Paper docs (0150 T3 and T4) comes after. | `files/download` refuses a Paper doc with 409 `unsupported_file`, and the source throws a bare error, so the row reads `source_refused` and the file is tried on five passes before it parks (0150 T1). The docs say Paper docs stay behind, while the code tries each one. |
...
| **M8. Dropbox native formats** (added 2026-09-26: 0150 D6, D7) | 1. 0150 T5 with T6 (a), a Paper doc refused by name and parked on its first attempt. 2. 0150 T9's lines in the Dropbox docs and guides, and its feature-matrix row. After the alpha: 3. 0150 T3, the format per kind, named at listing. 4. 0150 T4, the export. 5. 0150 T7 (a), Paper docs outside the file tree detected. | `dropbox-file-source.ts` and its types, `docs/dropbox-setup.md`, `docs/guides/*/dropbox.md`, `feature-matrix.md`; from step 3 also `config.ts`, the create and update routes, `CreateMapping.tsx`, `ExportPolicyPanel.tsx`, `strings.ts`, which R's groups touch (the out-of-turn rule below) | 0150 T2's listing of the owner's account before step 3 |
