...
> **In one line:** Umbrella for the managed alpha: its decisions (free, invite-only, Dutch, no backups), an alpha note on pages and grant mail, experimental labels on `SOURCE_CARDS`, Billing wording, end-of-alpha fate, go/no-go list for 0132 to 0150.

## Status — 2026-09-26 (update this block at the end of every session)

**2026-09-26, later: 0150 D9.** The owner answered 0150 open question 4 with (a): before 0150 T3 a
Paper doc states `policy_refused`, and the Failures page, in its rows and its group panel, chooses
the remedy by the migration's source as well as by its category, so a Dropbox migration is not
sent to Google's setting. So T5's 0150 row says so, and M8's step 1 now also changes the web app's
failure remedy and the failures payload: `strings.ts` (R1's), `failure-key.ts`, `Failures.tsx`,
`FailureGroupPanel.tsx`, the shared failures contract, and both editions' failures routes, managed's
among `apps/api`'s migration routes (R2's). M8 is no longer connector work alone, and the out-of-turn
rule applies from its step 1, not its step 2. Neither the 0150 row nor M8 waits on 0150 open
question 4 any more. The paragraph below, and its *"Nothing else in this plan changes"*, describe
the change before D9.

**2026-09-26: 0150 joins T5's list and §6's groups.** Workplan 0150 covers Dropbox Paper docs and
the other entries Dropbox lists as files but will not download. The owner's decisions on it were
recorded on 2026-09-26. Its alpha minimum (0150 D6) is its T5 and T6 (a), T9's lines in the
Dropbox docs and guides and its feature-matrix row, and a 0144 known-limitations entry, which is
written once 0144 T2's page exists, after the first invitation; until then the Dropbox guides
carry the limit. Exporting Paper docs comes after. So T5's table gains a 0150 row, and its range
reads 0132 to 0150. §6 gives the work to M, as M8, because it is connector work (0150 D7), and M
now works M1 to M8. Nothing else in this plan changes.

This plan holds §1 to §6 and its open questions twice: a second copy starts at the second
paragraph that opens *"Letting someone in is already one person's act."* The two copies already
differed before this note (only the second has open question 7 and its answered Billing
wording), and the 0150 row and M8 were added to the first copy only. Making them one copy is its
own change.
...
| T5 Go/no-go before the first invitation | 📋 **Proposed** | §3. For each of 0132–0150, 0093 T2c and 0130, the minimum that must be true, plus the owner's own steps. |
...
| 0150 Dropbox native formats (row added 2026-09-26, 0150 D6) | 0150's T5 and T6 (a): a Paper doc, and any entry Dropbox lists as a file but will not download, is refused by name with a stated category and parked on its first attempt. Until 0150 T3 lands, the refusal points to no setting: it states `policy_refused`, and the Failures page chooses its remedy by the migration's source as well, so a Dropbox migration is not sent to Google's setting (0150 D9). T9's lines: `docs/dropbox-setup.md`, the Dropbox guides and a feature-matrix row say what the code then does. The 0144 known-limitations entry is written when 0144 T2's page is built, after the first invitation; until then the Dropbox guides carry the limit (0150 D6). Exporting Paper docs (0150 T3 and T4) comes after. | `files/download` refuses a Paper doc with 409 `unsupported_file`, and the source throws a bare error, so the row reads `source_refused` and the file is tried on five passes and then waits on a person (0150 T1). The docs say Paper docs stay behind, while the code tries each one. |
...
works M1 to M8 (M8 added 2026-09-26 for 0150; its steps 3 to 5 come after the alpha), each group
in its numbered order.
...
| **M8. Dropbox native formats** (added 2026-09-26: 0150 D6, D7, D9) | 1. 0150 T5 with T6 (a), a Paper doc refused by name and parked on its first attempt, and its remedy chosen by source on the Failures page, in its rows and its group panel (D9). 2. 0150 T9's lines in the Dropbox docs and guides, and its feature-matrix row. After the alpha: 3. 0150 T3, the format per kind, named at listing. 4. 0150 T4, the export. 5. 0150 T7 (a), Paper docs outside the file tree detected. | `dropbox-file-source.ts` and its types, and from step 1 `failure-category.ts`'s comment, `strings.ts` (R1's), `failure-key.ts`, `Failures.tsx` and `FailureGroupPanel.tsx`, and the source kind on the failures payload (`operating-contract.ts`, managed's failures route among `apps/api`'s migration routes (R2's), and the appliance's in `apps/selfhost/src/index.ts`), D9; from step 2 `docs/dropbox-setup.md`, `docs/guides/*/dropbox.md` (R3's `docs/guides/`) and `feature-matrix.md` (R2's); from step 3 also `config.ts`, the create and update routes, `CreateMapping.tsx`, `ExportPolicyPanel.tsx`. From step 1 on, these are files R's groups touch (the out-of-turn rule below) | 0150 open question 1 (T1's evidence) before step 1; 0150 T2's listing of the owner's account before step 3 |
