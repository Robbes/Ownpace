...
| **M8. Dropbox native formats** (added 2026-09-26: 0150 D6, D7) | 1. 0150 T5 with T6 (a), a Paper doc refused by name and parked on its first attempt. 2. 0150 T9's lines in the Dropbox docs and guides, and its feature-matrix row. After the alpha: 3. 0150 T3, the format per kind, named at listing. 4. 0150 T4, the export. 5. 0150 T7 (a), Paper docs outside the file tree detected. | `dropbox-file-source.ts` and its types; from step 2 `docs/dropbox-setup.md`, `docs/guides/*/dropbox.md` (R3's `docs/guides/`) and `feature-matrix.md` (R2's); from step 3 also `config.ts`, the create and update routes, `CreateMapping.tsx`, `ExportPolicyPanel.tsx`, `strings.ts`. From step 2 on, these are files R's groups touch (the out-of-turn rule below) | 0150 open question 1 (T1's evidence) and open question 4 (the category before T3) before step 1; 0150 T2's listing of the owner's account before step 3 |
