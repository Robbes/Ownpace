...
|---|---|---|
| T1 an end-to-end proof that mail leaves the building | ✅ **Done 2026-08-15 — and it RAN** | Both `integration-tests` legs green on PR #398 (~3 min each), so the send genuinely executed: nodemailer connected to Stalwart over SMTPS 465 with the self-signed certificate, the message was delivered, and it was read back over IMAP with the body intact. The test THROWS on a missing env var, so green cannot be a silent skip. **This is the first time this code path has ever run.** | All three parts built: `SmtpSettings.allowSelfSignedCertificate` (REFUSED in production — the channel turns off with the reason named, and since T3 that is visible in `/status`); the harness publishes Stalwart's SMTPS 465 and exports `STALWART_SMTP_PORT`; `smtp-transport.integration.test.ts` sends through the real transport and reads the message back over IMAP, asserting the BODY carries the counts. A missing env var FAILS rather than skips. Mutation-verified on the guards: removing the production refusal, and loosening the exact-`'true'` match, each fail exactly one test. **NOT RUN — no Docker runtime in this container.** CI is the first place it executes, and the first time this code path runs at all. |
| T2 the managed cron task, tested like its self-host twin | ✅ **Done 2026-08-15 — the decision, and the position** | The two predicates that are NOT wiring — which tenants are considered, and who receives the mail — are extracted and pinned (`managed-digest-sql.unit.test.ts`, 3 tests). Mutation: widening recipients past owner/admin, and dropping the `tenant_id = $1` scope, each fail exactly one test. **The rest of the file stays deliberately untested wiring**, stated here rather than implied: importing it constructs a Pool and throws without `DATABASE_URL`, so a fuller test would assert its own mocks. |
| T3 make "notifications are OFF" visible to somebody who is not reading logs | ✅ **Done 2026-08-15 — payload and screen** | `StatusReport.notifications` carries enabled + the reason VERBATIM; `/status` supplies it (`index.ts`). 4 tests; mutation-verified — reporting a channel the caller did not supply, and summarising away the reason, each fail exactly one test. UI landed too: `NotificationChannelBanner` on the Dashboard renders only when the channel is OFF, shows the reason verbatim, and renders nothing when the payload omits `notifications` or the read fails — 4 tests, EN/NL. Mutation: treating "absent" as off fails all four; dropping the verbatim reason fails exactly one. |
