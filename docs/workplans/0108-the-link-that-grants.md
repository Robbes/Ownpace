...
reverses that. **A phone number is not built:** nothing stores one, and a number typed into a
profile is the member's statement, not a checked fact, so it would add an unverified line to the
one page that exists to be trustworthy. It waits on the owner's answer to that trade.
