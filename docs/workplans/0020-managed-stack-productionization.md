...
| T6 Task env vars stop being a dashboard ritual | 🟡 **Built, PR open — awaiting a live run** | `deploy/compose/set-task-env.sh`: reads the same `.env` the stack runs on, uploads `DATABASE_URL`/`APP_DATABASE_URL` (in-network `postgres:5432` addresses — runners join the compose network) + `SECRET_ENCRYPTION_KEY` via the SDK's `envvars.upload` with `override: true` (the .env file is the source of truth; a stale dashboard value silently winning is the failure mode this ends), then lists the env back as confirmation. Uses the SDK's own env contract (`TRIGGER_API_URL`/`TRIGGER_SECRET_KEY`) — no `configure()` call. Failure path self-tested (missing .env → FATAL, exit 1); the live upload needs the Spark stack. |
