...
Nothing is built. The fix to the runbook's manual recipe is drafted in the pending consistency
PR, which is not merged. It adds the identity provider's database and the cluster roles, and a
note that neither dump is usable without `.env`, that a copy of `.env` is kept off the host, and
that the managed recipe is not drilled.

| Task | Status | Notes |
|---|---|---|
| T0 The owner's steps on the reference machine | ⏳ **Owner** | §3. `BACKUP_RETENTION_DAYS=0` in the persisted `.env` the gate restores (and in the checkout's, if that is not a link to it), read back from the API container. T4's copies, if the owner takes them. Dates and outcomes go in this block, never values. |
...
| T4 The keys and the list of testers, once, off the machine | ⏳ **Owner** (recommended) | §3. A copy of `.env` that only the owner can open, taken after 0132 T5 and before the first tester connects. The list of testers, because the access queue that holds it would be lost too. |
| T5 Backups of both databases, encrypted, off the machine, drilled | 🅿️ **Parked (trigger: before the first paying customer, or when the alpha ends, whichever comes first)** | §3. Both databases and the roles, one retention number for the pruning and the erasure sentence, a restore drill in the managed gate, a dump before each migrating deploy, a stated RPO and RTO. |
...
- **The manual recipe** in `docs/operator-runbook.md`, *Backup & restore (§22.1)*, dumps and
  restores `-d openmigrate` only. It leaves out the `zitadel` database and the cluster roles. The
  fix is drafted in the pending consistency PR. The draft also says that neither dump is usable
  without `.env`, that a copy of it is kept off the host, apart from the dumps, and that the
  managed recipe has not been drilled.
...
- **What the architecture promises.** Its CI gates list *"**Backup/restore** drill (managed
  DB)"*. The architecture and `docs/deployment.md` promise *"DB backup before migrate"* for
  managed. Neither is built. 0132 T6 marks the backup before migrating as not built; the drill
  is T5 here.
...
drill for the Trigger.dev control plane"*), and each drill takes a real dump. The task payloads
the API enqueues for verification, deletion and relocation are ids, plus a hash for the last two
(checked in `operating-routes.ts`; the other enqueue sites were not checked). This plan did not
check what a task returns, or what a failed run records, in that store.
...
The reference machine's `.env` cannot be read from the repository, so this plan does not know
which of the two the OTA stack says today. T0 reads it. If the line is blank, the sentence names
backups that do not exist. The data is gone sooner than promised, not later, so the promised date
is kept. The sentence is still wrong about what exists.
...
way once a close screen exists (W14, 0131 §5). Every close records the number on
...
either for the reference deployment, and 0132 T6 replaces that procedure.
...
(*"PERSIST IT BACK"*, `e2e-managed.yml`).
...
- `ZITADEL_MASTERKEY` encrypts the identity provider's own data. The example says that
  *"replacing a live one strands every account"*. The repository puts it in `.env` (and so in
  the persisted copy and the gate's checkout) and on the `zitadel` service's command line, and
  nowhere else.
- None of `docs/managed-bring-up.md`, `docs/operator-runbook.md`, `docs/release.md`,
  `SECURITY.md` or `managed.env.example` mentions a copy off the machine. The pending consistency
  PR's runbook draft adds one sentence (above).
...
records (D1). Every place that suggests a backup exists is corrected (T1, T3), and testers are
told before they start, in Dutch first (T2, with D2 and D3). One machine carries the alpha and
CI (D4), so losing it ends both at once, and T3 says what then happens. Any copy of the keys
stays with the owner alone (D5, T4). The backups ADR-0015 asks for (*"Stack DR is ours"*) are
...
1. Set `BACKUP_RETENTION_DAYS=0` in the persisted `.env` under `~/.persistent/ownpace-managed`,
   which the gate restores (0132 §1). If the checkout's `deploy/compose/.env` is the link the
   bring-up asks for, that is one edit. If it is a separate file, set it there too. Setting it in
   the persisted file is what stops the next gate run from undoing it.
2. From `~/ownpace-managed`, recreate the API: `docker compose -f deploy/compose/managed.yml up -d
   api`.
3. Read it back: `docker compose -f deploy/compose/managed.yml exec api printenv
   BACKUP_RETENTION_DAYS` prints `0`.
4. If the owner accepts T4, take its copies after 0132 T5 and before the first tester connects.
...
- `managed.env.example`, the *Backups* block: the reference deployment takes no backups during
  the alpha and sets `0` (0134). Seven is the owner's number for when it does (0085 T5). Nothing
  in this repository backs up this database yet, so a blank line promises backups the stack does
  not make.
...
- `managed.yml`: the comment above `BACKUP_RETENTION_DAYS` says *"Unset means the 7-day default"*.
  It gains that the default assumes backups exist, and that the reference deployment sets 0 during
  the alpha.
- `docs/operator-runbook.md`, the `backupRetentionDays` row and the sentence *"The default is the
  reference deployment's"* under it: the default of 7 assumes backups exist, and the reference
  deployment sets 0 during the alpha.
...
- In production, an unset or empty `BACKUP_RETENTION_DAYS` is a warning. The warning names both
  ...
  they are.
- When 0131 T1's alpha setting is on, the same condition is fatal whatever `NODE_ENV` says, so an
  alpha stack cannot start while it quotes backups by default. It does not wait for production
  because `managed.yml` defaults `NODE_ENV` to `development`, and making it a required value is
  0132 T4, which is proposed.
...
personal data (an address, a folder name, a provider's error text), then the Trigger.dev dumps
are backups in the erasure sentence's sense. `BACKUP_RETENTION_DAYS` must then cover how long a
...
| The ledger (`item`, cursors, status) | `openmigrate` on `ownpace-db` | Lost. A re-created migration's first pass adopts what the target holds, and reads the whole source again once. |
...
| Every sign-in identity | `zitadel`, same server | Lost. |
| The keys and passwords | `.env` | Lost, unless T4. |
| The Trigger.dev account, project and keys | `triggerdb` and its dumps, same machine | Lost. Recreating them *"needs a person and a browser"* (`trigger-version.sh`). |
...
|---|---|---|
| The whole machine | Every tester starts again (T2). | The same for testers. The owner keeps the OAuth client secrets, the mail login and the rest of `.env`. The two keys protect nothing any more and can be replaced at no cost. |
| `.env` only (a deleted persisted directory, a file overwritten) | Every stored credential is unreadable, so every tester reconnects every account. Every identity is stranded, so the identity provider's database is started again and every tester registers again. Organisations, migrations and ledgers survive. Whether a person who registers again is matched to their old membership (`tenant_member` keeps both `user_id` and `email`) was not checked. | Nothing is lost. |
| The database only | Every tester starts again (T2). | The same. |

The second case is the one T4 is for. It loses no tester data, and without the copy it costs every
tester more than the first case does. The gate rewrites the persisted `.env` on every run, and
the repository already records one key the deployment no longer held (§1).
...
- `docs/operator-runbook.md` gains *If the machine is lost during the alpha*, with the owner's
  ...
  2. Bring up a new stack by the bring-up guide, with T4's copy of `.env` if there is one. If the
     old machine could be in someone else's hands, replace every value: nothing depends on the old
     ones any more, and the OAuth client secrets are replaced in each provider's console.
  3. Do Trigger.dev's one-time steps.
  4. Grant each tester again as they come back.
  5. Record the date and what was lost in this plan's Status block.
- The runbook's *Backup & restore* section gets one opening sentence: the reference deployment
  takes no backups during the alpha (0134), and the recipe is for a deployment that does. The
  pending consistency PR's draft already says the managed recipe is not drilled.
...
**The copy of `.env`.** After 0132 T5 has given the stack its final values, and before the first
tester connects, copy the persisted `.env` once to a place off the machine that only the owner
can open. It must be an encrypted file, and not in the repository, a CI secret, a chat or a mail
(D5). Keep it apart from any database dump (T5), so that no single thing lost or stolen is both
the data and its key.

- Refresh the copy whenever a value in `.env` changes, and write the date of each copy in the
  Status block.
- To check that the copy still matches without printing either file, decrypt it to a temporary
  file and run `cmp -s` against the live one. It answers same or different, and the temporary
...
- **A script with the verbs `trigger-version.sh` already has.** `deploy/compose/app-db-backup.sh`
  ...
  - It dumps `openmigrate` and `zitadel` from `ownpace-db` with `pg_dump --format=custom`, and the
    cluster roles with `pg_dumpall --roles-only`. Without the roles, a restore dies on the first
    `GRANT` (`selfhost-quickstart.md`).
...
- **A schedule.** It runs daily from 0132 T7's timer (`box-duties.sh`).
- **A dump before each migrating deploy.** It becomes a required step of 0132 T6's procedure.
...
T0 first: it takes minutes and makes the erasure sentence true on the machine the same day. Then
T1's wording and start-up check, in one PR, and T1's check of the Trigger.dev store. T2 goes to
...
refusal sentence in `migrate.ts`. T4 comes after 0132 T5 and before the first invitation. T5
waits for its trigger.

## Not in this plan

- The deploy procedure and whether it includes a dump: 0132 T6, and open question 1 here.
- The timer that keeps the gate's duties running: 0132 T7.
- Replacing the demo-era secrets: 0132 T5. T4's copy is taken after it.
...
- Being told that the machine is down: W12 (0131 §5).
...
1. **A dump before each deploy (0132 open question 4).** (a) None. This is D1 read literally:
   ...
   lost machine does, which T2 already tells them. *Recommended for the alpha.* (b) A dump of both
   databases before each deploy, kept on the machine until the next deploy succeeds and never
...
2. **T4: take the copy of `.env` and the list of testers off the machine?** Recommended: yes,
   ...
   rules.
3. **The Trigger.dev dumps during the alpha.** Keep them, as the orchestration plane's rollback
   aid, if T1's check finds no tester data in the store? Or stop them for the alpha? Keeping them
   is recommended, because that store is what *"cannot be rebuilt unattended"*. If T1's check does
   find tester data, `BACKUP_RETENTION_DAYS` must cover how long they live. With the gate off (0132
   T1) and no timer yet (0132 T7), a dump lives until someone runs the script seven more times.
