...
|---|---|---|
| T1 The decision plumbing (write, list, resolve) | 🟡 **Store half built 2026-08-03** | Migration 0005 gives `decision` a `subject_key` + the partial unique index that makes raising idempotent AT THE DATABASE (pending-only — an answered subject may be asked again; history accumulates). `DecisionStore` port in `@openmig/shared`; `PgDecisionStore` in the ledger (raise via on-conflict + read-back with a one-retry gap guard, list newest-first with status/mapping filters, resolve/dismiss that never overwrite an answer — the second answer gets `undefined`). 7 PGlite-backed unit tests (real migrations applied). Gates: typecheck + lint clean, 1298/1298 unit. Remaining for T1: the routes in both editions + the screen skeleton reading an empty queue honestly. |
