...
| T0 The probe-qualified record | ⬜ proposed | For Basic-auth account families (DAV/IMAP/JMAP — Soverin, Nextcloud, Fastmail, Stalwart): at connection test, probe EVERY protocol face the account could wear — CalDAV discovery, CardDAV discovery, IMAP LIST, WebDAV PROPFIND — and store a per-domain verdict on the connection: `{mail, calendar, contact, file}` each `yes \| no \| unknown`, plus what T0-of-0105 already measures (the scheduling verdict) folded in. Three states, never two: UNKNOWN is unmeasured-not-safe, never a quiet no (the run-#6 rule). Read-only by construction — every probe is a `listFolders`-class question. Recorded like `calendar.target_scheduling`: an audit event plus a field on the row, re-measured on every re-test so the record says what was LAST true. Both editions: the appliance's config-file connections get the same record from `verify`/startup probing and surface it in the §14.2 report; no UI needed to hold a fact. |
