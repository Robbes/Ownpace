...
> **The file/WebDAV domain gap flagged when T7 closed has now also been fixed and verified live**
> (PR #119, same branch, commits `e99a9b6`–`60c5e95`) — see "Round: file/WebDAV domain gap closed"
> immediately below. **All four domains (mail, calendar, contact, file) now have real, externally-
> verified evidence of end-to-end sync on this branch.** No open gaps remain from T7.
>
> **Round: file/WebDAV domain gap closed (2026-07-26, PR #119)** — plain WebDAV has no RFC 6764-style
> discovery the way CalDAV/CardDAV do, so a managed-edition tenant sharing one Nextcloud `connection`
> row across calendar/contact/file had no way to resolve the file domain's actual
> `.../remote.php/dav/files/{username}/` root from the shared bare base URL. Fixed with
> `fileEndpointFromCreds()` (`apps/worker/src/dav-endpoint.ts`) — an explicit `config.fileBaseUrl`
> override, else Nextcloud's own `files/{username}/` convention appended for `kind: 'nextcloud'`,
> else passed through as-is for a dedicated `kind: 'webdav'` connection. That alone fixed source-side
> discovery (zero PROPFIND errors, versus the previous constant 405s), but three more real bugs
> surfaced chasing it to a genuine end-to-end write, found via live debugging on the Spark box
> (never just from logs — each confirmed by direct PROPFIND/GET against the real Nextcloud target):
> 1. **`WebDAVTargetWriter` path-doubling + swallowed PUT failures.** `ensureCollection` passed the
>    *source's* own folder object straight through to the target writer's `ensureDirectory`, and the
>    natural key was the source's own full absolute href — concatenating them produced a garbled,
>    always-404 target URL (confirmed: a real ledger row with `target_ref` containing a doubled
>    `.../tenant-b-source/remote.php/dav/files/tenant-b-source/...` path). Worse, `uploadFile()` never
>    checked the PUT response status, so the failed write was recorded as a false `'copied'` success,
>    which then permanently blocked retries via the ledger's own fast-path. Root-fixed by making
>    `WebdavFileSource` emit genuinely root-relative paths (matching `FileItem.path`'s own doc
>    comment, which the old absolute-href behavior silently violated) via a new `toRelativePath()`/
>    `basePathname()` pair, and having `WebDAVTargetWriter` resolve that relative key directly against
>    its own root — no parentId concatenation possible. Added a PUT/chunked-PUT status check matching
>    `CalDAVTargetWriter`/`CardDAVTargetWriter`'s existing pattern.
> 2. **`listSince()` ignored `config.rootPath`.** The Rule-B (config-derived) URL builder only
>    appends to `config.url`, but `rootPath` is a separate field some callers set (the WebDAV
>    integration test harness, and the file-domain readback source) — caught by CI's real
>    `integration-tests` job 404ing against a real Nextcloud. Fixed with `combineWithRootPath()`,
>    the inverse of `toRelativePath`/`basePathname`.
> 3. **Root-folder trailing-slash mismatch in `toRelativePath`.** Confirmed live: a depth:1
>    PROPFIND's *self*-entry for the account root can come back **without** a trailing slash, while
>    every other collection href (and `basePathname()`'s own construction) has one — the exact-prefix
>    check then failed for the root specifically, resolving it to almost its entire absolute path
>    instead of `''`, which fed back into the exact same doubled-path bug as #1 but for the root
>    "folder" itself. Found by reproducing the real `WebdavFileSource.listFolders()` call live via
>    `tsx` inside the worker container (not guessing from logs) — it returned
>    `"remote.php/dav/files/tenant-b-source"` instead of `""` for the root entry. Fixed by checking
>    both trailing-slash variants of the root explicitly; added a regression test.
>
> **Verified live end-to-end after all four fixes** (Spark box, `pr-57-draft` @ `60c5e95`):
> `managed-scheduler` log line `b0000000.../file: 1 created, 64 skipped` with **zero errors**; the
> `item` table now has correctly-shaped, non-garbled `target_ref` values (e.g.
> `{"id":"Documents/Readme.md"}`); and a direct `GET` against the *target* Nextcloud account
> (`tenant-b-target`) for `Documents/Readme.md` returned `200` with the real seeded file content —
> proving the file actually landed on the target, not just that the ledger claims it did. CI:
> all 9 checks green on `60c5e95` including the self-hosted `integration-tests` job.
>
...
| T4 usage metering from real runs | ✅ Done | **Merged PR #50.** `packages/ledger/src/usage-metering.ts` defines the §16 drivers: `recordComputeForRun` (run minutes), `recordApiCallForRun` (sync ops), `deriveStorageAndEgressForPeriod` (bytes/storage derived from the immutable `item` ledger), `getUsageMetricsForPeriod`. Idempotent via upsert keyed by `(tenant_id, period_start, metric_type, resource)` — retries/re-recording are a no-op, never a double-count. **Corrected 2026-09-08 by [0121](./0121-a-month-billed-as-one-pass.md):** the key had no PASS in it, so each pass overwrote the last and a billing period recorded one of its 2 976 passes. `resource` is now `domain-<domain>#<runId>` / `sync-<domain>#<runId>`, the same upsert on a narrower key — retry-safety kept, the period's compute their sum. The module moved to `packages/managed/src/usage-metering.ts` with ADR-0036. Wired into `run-delta-sync.ts` (records inside `withTenant` after each mail pass, using `migration_status` timing). RLS-scoped. **Tests:** `packages/ledger/src/usage-metering.integration.test.ts` (exactly-once, re-run no-op, RLS isolation). Design/ground-truth in `docs/design/0011-t4-metering.md` + `docs/design/0011-t4-metering-ground-truth.md`. **Gates:** lint + typecheck green. |
...
> **What that acceptance could not see, found 2026-09-08 (0121).** All three properties held
> and the number was still wrong: keyed per period, "record two passes and read" and "record
> one pass twice and read" are the same query, so exactly-once and no-op-on-re-run pass
> identically whether the key names a pass or a month. The missing acceptance was *a period
> with more than one pass in it*, which is every real month.
