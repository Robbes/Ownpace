...
> **T1–T6 done and merged** (PRs #43–#50, #52–#56). **T7 is 🟡 mostly done** — the sync pipeline
> is proven end-to-end for the first time, and the fake `/status` endpoint is now fixed for real
> (see T7 row below). What's left is confirming RLS isolation, billing/usage recording, full
> service health, and a non-zero-item sync against a live stack — none of that needs more code,
> just someone with Docker access to run it and paste the evidence.
>
