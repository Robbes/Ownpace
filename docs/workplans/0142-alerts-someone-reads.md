...
writes a runbook for a bad day (T6), stops a lane that checked nothing from showing green (T7),
and corrects the architecture document (T8). Dashboards and SLOs stay parked (T9).

**The minimum before the first invitation is T0, T1, T2 and T6** (§4). The rest follows the
first invitation.

Nothing is built. Three neighbouring fixes are drafted in the pending consistency PR #1137, and
none is merged:
...
| T1 The status page tells the owner when an Ownpace row goes red | 📋 **Proposed** (D1) | §3. **Alpha minimum.** An `alerting` block in `gatus.yaml`, with an address and a switch, as the Website row already has. Alerts on the Ownpace rows only. |
| T2 A tick that says it ran | 📋 **Proposed** (D1) | §3. **Alpha minimum.** The tick writes one row each minute. `GET /api/ready/scheduler` reads it, and a status row with an alert reads that route. `/api/ready` stays as it is. |
...
| T6 What to do when an alert or a tester says something is wrong | 📋 **Proposed** (D1, D2, D4, D5) | §3. **Alpha minimum.** `docs/incident-runbook.md`: a row for every alert, the steps when a tester reports trouble with their data, and the hand-over to 0139 T8's breach procedure. |
| T7 A lane that checked nothing is not green | 📋 **Proposed** | §3. After the first invitation. The live-target lane stands down unarmed, and the run should look skipped, not passed. |
...
- The API's `/metrics` says what it does not do (`apps/api/src/index.ts`): *"What this does NOT
  ...
  decision"*. Managed passes do not feed it anyway. `recordPassMetrics` is reached only through
  ...
  process.
- The daily digest (`apps/worker/src/jobs/managed-digest.ts`) goes to each organisation's owners
  and admins, as `tenant.settings` says. It does not go to the operator.
- `NOTIFY_TO` is the operator's address. Today the product uses it for one message only: the
  notice that somebody asked for access (`apps/api/src/access-notify.ts`, 0133 §1). On the OTA
  stack that mail goes to Mailpit until 0133 moves it to a relay.
...
through `STATUS_URL`, which defaults to `http://gatus:8080`).

The sign-in page links to the page. `StatusLink.tsx` turns `app.` into `status.`, so
`app.ota.ownpace.eu` links to `status.ota.ownpace.eu`. Its own comment says that *"A link to a
status page that does not exist is worse than no link, because it answers 'is it down' with a
browser error"*. The review recorded the status page as reachable only over the mesh. That cannot
be seen from the repository. 0132 T3's outside probe tries `app.`, `id.` and `www.`, but not
`status.`.
...
for a server connection. Its comment still calls it a statement limit; fixed in #1137 (pending
merge).
...
report Zammad refused (`report.not-delivered`). The operator's log page shows these rows (0129
T2). It shows them only when somebody opens it.

**A green lane that ran nothing.** `e2e-live-target.yml` is scheduled for 04:30 UTC. GitHub's run
list, re-checked on 2026-09-24, shows 28 runs, all scheduled and all `success`. GitHub started
them between 08:27 and 11:14 UTC, and most finished in about thirty seconds. The review read the
latest job's log: *"live-target: STAND-DOWN — unconfigured, and says so"*. That was not re-read
here.
...
loudly-but-green"*. The decision file says why: *"red would train everybody to ignore the lane
before it exists; silent green would claim a proof nobody ran"*. 0105's status marks T4 *"Built
...
#1137 corrects the document's CI gates, not §18 or §19.
...
So the stack, its status page, CI and every check in T1 to T4 share one machine. When the machine
or its connection is down, all of them are down together. T5 is the only answer to that, and §4
says what stands in for it during the alpha.
...
- **(a) E-mail** to the address in `NOTIFY_TO`, through the relay 0133 T0 sets up. *Recommended*
  once 0133 T3 has moved the product's mail to that relay: the login already exists, and Gatus
  has an e-mail provider.
...
**The steps, before the first invitation, while nobody but the owner uses the stack:**

1. Choose (a) or (b), and put its settings in the persisted `.env` (T1 names them). Values stay
   out of this plan.
2. Send one test alert. Stop the web container for four minutes
   (`docker stop ownpace-web`, then `docker start ownpace-web`). The rows that read the public
...
3. Check from a machine off the mesh whether `status.ota.ownpace.eu` answers. The easiest way is
   to add it to 0132 T3's outside probe. Write the outcome here, and see open question 4.
...
log. It answers `off` where no managed tick runs, as on the appliance.

A new Gatus row, **Scheduled syncs**, in the Ownpace group, reads `[BODY].scheduler == up` through
`STATUS_WEB_URL` and carries T1's alert. Because the operator's support screen reads Gatus's JSON,
...
reference machine. The unit files go in the bring-up, beside 0132 T7's. It measures free space on
...
- Free space.
- The size of the application database, the identity provider's database and the Trigger.dev
  plane's database (`pg_database_size`).
- The five largest volumes (`docker system df -v`).
- Row counts for `run`, `run_event`, `item` and `app_event`.
- The age of T2's beat.
...
**A voice for 0132 T7.** 0132 T7's `box-duties.sh` *"exits non-zero and names the duty that
failed"*, and 0132 says *"Nobody is told when it fails; that is outside this plan."* This plan
tells the owner: the timer's unit runs `tell-owner.sh` when the duty fails, using systemd's
`OnFailure=`.
...
**The queue.** The tick already talks to the task plane each minute. It reads `queues.retrieve`
for `delta-sync` and `run-discovery` and stores `queued` and `running` in T2's row, beside its own
summary numbers (`triggered`, `heldBack`, `staleRuns`, `failedToEnqueue`). A failed queue read
costs the numbers, never the beat.
...
**The pooler.** `box-checks.sh` runs `SHOW POOLS` in `ownpace-pgbouncer`, as the healthcheck does.
...
`sync.<domain>.failed`, `api.<code>` and `report.not-delivered`. This is a count, not an alert. A
tester's expired credential is the tester's to fix, and their digest tells them. The owner sees
from the count whether the log page (0129 T2) is worth opening today.
...
this one"*. It uses public addresses:
...
started this repository's 04:30 lane between 08:27 and 11:14 (§1). Its logs are public, and it is
US-hosted, which `docs/status-page.md` asks the status page not to be.
...
If the off-box copy becomes the public page at `status.ota.ownpace.eu`, the sign-in page's link
reaches testers (open question 4).
...
- a tester's report, through 0130's form where a Zammad is configured, or by mail to the address
  in the conditions;
- GitHub's mail about a red nightly.
...
- *Database*: `docker ps`, then `docker logs ownpace-db`, then the pooler.
- *Sign-in* and *Identity provider*: the bring-up's failure table.
- *Scheduled syncs*: the supervisor's log and Trigger.dev's run list for `managed-sync-tick`.
...
3. **Stop it spreading.** If other testers could be touched, or anything could be removed, start
   the hold with a Dutch sentence. It stops scheduled passes and is shown to every signed-in
   tester. It does not stop a pass started by hand (0132 T6 proposes that it should). So ask the
   affected tester to pause the migration (the *Pause* action on the migrations list) and not to
   press *Sync now* until told.
4. **Keep the evidence.** Do not redeploy, restart, prune or reset before the relevant rows and
   container output are copied off the machine (0139 T8 step 2). If the nightly managed gate still
   runs, switch it off first (0132 T1).
...
### T7 — a lane that checked nothing is not green

The lane's stand-down is honest in its text and misleading in its colour: 28 green ticks, none of
them evidence (§1). Red is not the answer, and `live-target-lane.ts` explains why. The answer is
grey.

**Route (a), recommended.** Add a repository variable, `LIVE_TARGET_LANE`. It is not a secret and
holds the word `armed` and nothing else. The job runs only when it is set
(`if: vars.LIVE_TARGET_LANE == 'armed'`), so an unarmed lane shows as skipped.

When the variable is set and the persisted `.env` has no `LIVE_*` lines, `runLane` returns
`ok: false`. The step passes it `LIVE_TARGET_LANE` so that it can tell the two cases apart.
Somebody said the lane was armed and it is not, and the lane already treats a half-set-up checker
as a mistake. The credentials stay where 0105 put them. The variable says only that they are
meant to be there.

Before the guard is written, confirm with one dispatched run, with the variable unset, how
GitHub shows a run whose only job was skipped. If it shows green, use route (b).

**Route (b).** The job posts a check run with conclusion `neutral` and the stand-down line as its
title. This needs `permissions: checks: write`. The workflow run itself stays green, but the
commit's checks show neutral.

**The words.** 0105's status row gains *"not armed: stands down every night until T3's sitting"*,
in the same PR. The workflow's Node version is fixed in #1137 (pending merge).

**Guard.** `scripts/a-lane-that-checked-nothing-is-not-green.unit.test.ts` checks:

- the job carries the `if:` (route a);
- the workflow hands `LIVE_TARGET_LANE` to the lane's step;
- `runLane`, with the catch-all off and the lane declared armed, returns `ok: false`.

It fails today on all three.
...
- T1 to T7, each added as it lands.
...
For 0131 T5's go/no-go table, this plan's row is: T0's test alert arrived and its recovery too;
T1 and T2 are on the alpha stack with the switch on; T6 is in `docs/`. As with the other rows, the
owner may accept a gap in writing instead, dated, with the reason.
...
4. **T7**, which changes how the owner reads CI, not what a tester meets.
5. **T8**, in the same PR as T1 or straight after.
...
- **The breach procedure, the record of processing and the security policy's channel:** 0139 T8
  and T9. That includes 0026 row 11's threat model, and `SECURITY.md`'s sentence calling it an
  open decision.
- **Arming the live-target lane:** 0105 T3's supervised sitting, and the live runs in 0141.

## Open questions

1. **The channel (T0).** (a) E-mail to `NOTIFY_TO` through 0133's relay, which is recommended once
   that relay carries the product's mail; or (b) a chat or push webhook hosted in the EU, if the
   first invitation comes first. Alerts can arrive at any hour. Is it acceptable that nothing wakes
...
3. **The Scheduled syncs row (T2):** on the public page, where it tells a tester why nothing
   ...
   checked), "kept to the owner" means T3's script reads the route instead of the page. The recommendation is the public page, with the
   plain name, and with `/api/ready` left alone.
4. **The status link on the sign-in page.** If `status.ota.ownpace.eu` does not answer off the
   mesh (T0 step 3), the link answers testers with a timeout. `StatusLink.tsx` itself calls that
   worse than no link. Should T5's off-box copy become the public page at that name, or should
   the link be hidden on the alpha stack until it does?
5. **T7's route.** (a) A repository variable, so that an unarmed lane is skipped; or (b) a
   `neutral` check run beside a green workflow run? (a) is recommended, if one dispatched run
   shows it grey.
