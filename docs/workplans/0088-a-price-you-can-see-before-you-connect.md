...
| T6 Name the three rungs, and what a free preflight may keep | ✅ **Decided and built 2026-09-20** (owner: "7 days is fine") | Naming the three rungs is documentation and could be written today. **Stating the retention rule for non-customers is not.** A free preflight reads somebody's mailbox to count it, and how long we keep those counts for a person who never becomes a customer is a privacy commitment, not a description of current behaviour: whatever gets written down becomes the promise, and it is the owner's to make. Writing a plausible one and calling it documented would be the worst outcome — a retention policy nobody decided, cited later as though somebody had. So this stays open with the question named rather than half-done: **how long may a free preflight's discovery counts survive for somebody who does not sign up, and what deletes them?** **Answered 2026-09-20:** a person who connects a source for a free estimate and never becomes a customer — the credential is discarded the moment the count finishes and is never stored beyond it; the discovery counts live **seven days**, tied to the estimate, and a scheduled job deletes them; nothing is ever mailed from them. **Built the same day:** `prunePreflightCounts` in `packages/ledger/src/retention.ts` (the mapping still `paused`, no pass ever run for the tenant, and not a tenant the caller names as a customer — the managed job passes its invoiced tenants, the appliance does not run it), wired into the nightly `managed-retention` task with `PREFLIGHT_RETENTION_DAYS` uploaded beside its two siblings; the privacy text's row said **30 days** while nothing deleted anything — it now says seven, in both languages, with a sentence in §4.3, and the estimate page says it beside the form. Tests on PGlite for each of the three conditions and the window; a source guard on the wiring. **Not built, and asked of the owner:** the credential half — the privacy table keeps credentials until the migration ends or the person deletes them, which is what the product does; the decision's wording had them discarded when the count finishes. |
