...
| T3 The static page | 🚧 **Blocked on an owner call (found 2026-08-26): the calculator collides with the site's no-script CSP** | The public site is served with `default-src 'none'; … form-action 'none'` (`deploy/compose/www-nginx.conf`) — no scripts and no submissions, deliberately; the request-access form moved to the app for exactly this reason (`site/build.mjs`, `orderHref`). A five-input live calculator needs JavaScript, so one of three shapes must be picked: **(a) one hash-pinned inline script on the calculator page alone** — CSP for that page allows exactly its own script by sha256, nothing external, `form-action 'none'` stays (the calculator submits nothing), and the posture sentence updates from "no scripts" to "no scripts except the calculator's own, hash-pinned"; **(b) a build-time precomputed, CSS-only calculator** — every discrete combination (3 customer types × object-type ticks × durations) rendered at build and revealed with `:checked`, no script at all, but "how much" degrades from an editable number to preset bands, which dilutes T5's editable-assumptions requirement; **(c) host the calculator on the app** — but rung 1's whole point is no account, and the app is invite-only. Recommendation: **(a)** — it keeps the spirit (nothing external, nothing submitted, everything auditable) and delivers the plan's actual UX. T2's data (`profiles.mjs`) is built and waiting either way. |
