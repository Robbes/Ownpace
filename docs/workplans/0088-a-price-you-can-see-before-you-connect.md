...
| T6 Name the three rungs, and what a free preflight may keep | ✅ **Decided 2026-09-20, twice** — a seven-day rule for "people who never become customers" was built and merged in the afternoon (#1030, owner: "7 days is fine") and **withdrawn the same evening** (owner: "withdraw") once it was re-read against the product | Naming the three rungs is documentation and could be written today. **Stating the retention rule for non-customers is not.** A free preflight reads somebody's mailbox to count it, and how long we keep those counts for a person who never becomes a customer is a privacy commitment, not a description of current behaviour: whatever gets written down becomes the promise, and it is the owner's to make. Writing a plausible one and calling it documented would be the worst outcome — a retention policy nobody decided, cited later as though somebody had. So this stays open with the question named rather than half-done: **how long may a free preflight's discovery counts survive for somebody who does not sign up, and what deletes them?** **Answered 2026-09-20, the reading that stands:** there are no strangers. Every preflight runs behind a sign-in (`POST /api/migrations/:id/discover` is authenticated), in a tenant the owner provisioned after an access request (0092 T4); no free tier exists; nobody picks a tier, the preflight derives it. So **(1)** a person with an account keeps their preflight counts *with the migration they were counted for*, and never longer than the account — the privacy table's account row (while it exists, then 30 days) and its unchanged credential row are the whole retention; **(2)** an account-less preflight is not offered, and if rung 2 is ever built without an account the rule is fixed now: the credential is used for the count and never stored, the result is shown and not kept. **The afternoon's build was withdrawn the same evening:** `prunePreflightCounts`, `PREFLIGHT_RETENTION_DAYS` and the "7 days if you never become a customer" row came out again, the privacy text (§4.3, §9, both languages) and the estimate page now say the counts stay with the migration and no longer than the account, and the credential half of the afternoon's question dissolves with it — the closing erasure already revokes, and the delete button revoking is its own fix (the privacy text promises it; the route never called it). |
...
**Answered 2026-09-20.** The aggregates-only half holds: `migration_discovery` stores
collections, items, bytes and per-collection tallies, never an item identifier (the
identifiers reach the ledger at the first real pass). The expiry half was decided in two
readings on one day — see the T6 row. What stands: a preflight is only ever run by somebody
with an account, so its counts follow the migration and the account, and no separate expiry
exists; a preflight without an account, if ever offered, forgets on showing the result.
...
- The retention machinery in T6 — decided 2026-09-20: there is none to build. A preflight's counts follow the migration and the account (T6 row).
