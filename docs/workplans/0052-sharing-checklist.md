...
| T6 what remains, honestly | ⛔ **Not built, deferred within the accepted decision** | (a) Digest narration of sharing counts; (b) the completion report's "access carried over" section (applied / done by hand / skipped / still open); (c) confirm-once-apply-to-all-rows grantee address mapping (today the address is confirmed per row); (d) a real-Nextcloud integration test of the OCS call (unit-tested against its documented envelope; first live proof rides the owner's runbook); (e) group shares and DAV ACLs (out of ADR-0032's scope by its own words). |
