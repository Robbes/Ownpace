...
> **Near-done.** T1/T3/T5/T6 done and tested; **T2 is done except its `--yes` approval gate**,
> which a doc audit on 2026-07-27 found was never implemented despite the row claiming it (see the
> T2 row). **Owner decision 2026-07-16: verify-only DNS** — T4 (automated provider writes) stays
> deferred; verify-only is the permanent cutover DNS story.
>
> **Open:** add the `--yes` confirmation gate to the `rollback`/`approve`/`execute` CLI
> subcommands. That is the last item in this plan.
...
| T2 cutover state machine persisted + driven by worker | 🟡 **Done except the `--yes` gate** | PR #31 merged (56f4a50); commits: ffde0c9, 8672893, 37f500c (state machine fixes), 96c6249 (state-machine tests), 34dbb0a (complete cutover), 765abc7 (foundation), 35956b0 (integration tests Steps 8-10); persistence via `packages/ledger/src/cutover-store.ts`; CLI via `apps/worker/src/cli/cutover-commands.ts`. **Correction 2026-07-27:** this row previously claimed the full task including the "behind explicit `--yes` confirmation" requirement. That part was never implemented — `rollbackCutover()` prints `Confirm rollback? …` and then proceeds unconditionally (`// In a real CLI, we'd prompt for confirmation / For now, we'll proceed`), and `approve`/`execute` have no gate at all. Persistence, rehydration, transitions and the event log **are** done and integration-tested; the approval gate (§11.2 control actions, hard rule 2) is open. |
