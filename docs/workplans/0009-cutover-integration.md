...
> **In one line:** Cutover lifecycle: the verification gate and its count-only PASS, the persisted `cutover_state` machine, worker CLI and row security, verify-only DNS checks and runbook, rollback and retry, and the managed cutover endpoint, job and smoke.
