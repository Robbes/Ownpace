...
**2026-09-24: opened from the owner's answers.** The readiness review of 2026-09-23 found five
things about versions and releases. The only release is `v0.1.0-rc.1`, of 2026-08-04, and every
build since calls itself that. The changelog stops in the middle of August. The managed stack
runs whatever commit was last checked out on the machine. The tasks run on Node 21, while
everything else runs Node 24. And several rules in the repository hold only "pre-release", which
nothing says when ends. The owner chose to have this plan written: *"W11 write, W12 write, W13
write, W14 write, W15 explaoin, W16 write, W17 write, W18 explain, W19 write"*. 0131 §5 calls this
work W17.
...
- T6, the tasks run the Node major the images run, and a pull request shows the task bundle loads
  there.

**After the first invitation:** T1, T3, T4 (apart from the drill from rc.1, which T2 runs because
`docs/release.md` requires it before a tag), T7 and T8. T7 is one sentence from the owner, and is
best given with T0.
...
| T6 The tasks run the Node the images run | 📋 **Proposed** | §3. **Alpha minimum.** `runtime: 'node-24'` in `trigger.config.ts` (supported by the pinned CLI, read from its package), the version guard extended, and a pull-request check that the bundle loads. |
...
Each fact below was checked at the current checkout (`main` at the merge of #1137) on 2026-09-24,
...
- **Where the number shows.** The API's `/version` and `/api/version` (`apps/api/src/index.ts`,
  ...
  the bottom of the sidebar (`Layout.tsx`, lines 337–338), and on the sign-in and invitations pages
  too. It shows the UI's build and the API's build separately when they differ
  (`apps/web/src/services/build-identity.ts`, `describeBuild`).
- So **every managed build since 2026-08-04 reads `v0.1.0-rc.1 · <commit>`**, and only the commit
  tells two builds apart.
- **A problem report does not carry the build.** The ticket 0130 builds lists the page, the
  reference, the category and the organisation, and nothing else (`ticketFor` in
  `apps/api/src/problem-report.ts`, lines 146–154).
...
- **The procedure** is `docs/release.md`, in four steps:
  ...
  - the tag: `git tag -a`;
  - what the tag fires: `images.yml`, `security-scan.yml` and `windows-payload.yml`;
...
- **Nothing checks that the names agree.** No workflow compares the tag with `package.json`, or
  looks for the tag's changelog section. `security-scan.yml` reads `GITHUB_REF_NAME` only to write
  the release body (lines 228–230). A tag pushed without the bump would publish images tagged with
  the new version whose `/version` says `0.1.0-rc.1`.
- **`rc.2` would send the drill to the wrong registry.** ADR-0040's operative rules say: *"An image
  ...
  `scripts/upgrade-drill.sh` derives its registry from the tag (lines 52–55):
  ...
  `ownpace-selfhost`, but the drill would look for it under the old name, where it does not exist.
...
- `scripts/squash-migrations.sh`, lines 17–21: *"ONLY SAFE PRE-RELEASE. Squashing rewrites history
  ...
  - Its only switch is `--apply` (lines 40–41). It works on a scratch server named by `ADMIN_URL`,
    so it cannot see any deployment's tenants. Nothing in it refuses.
...
- `README.md` line 103: *"Active development, pre-release."* No document says when pre-release
  ends. A grep of `docs/*.md`, `docs/adr/`, `README.md`, `SECURITY.md` and `AGENTS.md` finds three
  other uses of the word: `release.md`'s SemVer meaning, `deployment.md`'s channel rule and
  ADR-0036's heading.
...
- **From source, not from the release images.** `managed.yml` builds the API and the web app from
  the checkout (`build: context: ../..`, lines 782–785 and 953–956), with `GIT_SHA` (lines 791 and
  962). No service in it pulls an `ownpace-*` image. `images.yml`'s signed images are the
  appliance's channel.
- **The web image cannot simply be swapped in.** It bakes `VITE_OIDC_ISSUER` and
  `VITE_OIDC_CLIENT_ID` in at build time (lines 970–971), so a published web image is not a
...
- #1137's version guard now says so in its header
  ...
  it.
- No CI job bundles the task files, or imports them under any Node.
...
- **Trigger.dev.**
  ...
  - `deploy/compose/trigger-version.sh list` probes upward for newer tags that both images carry
    (lines 161–196), when somebody runs it.
...
- **ClickHouse.**
  - It is pinned at `26.2.19.43` by digest, *"as read from trigger.dev's own
    docker/docker-compose.yml at v4.5.12"* (lines 272–275).
...
- **MinIO.**
  ...
  - 0119 §3 item 5 (lines 228–241) says *"There is no maintained free MinIO image to move to"*.
    It names versitygw as the candidate, and Garage as the heavier alternative.
...
- **The owner role's name.** The owner reports that the database user names were changed on the
  machine (0131 D3: *"Usernamea changed"*). That is a setting in a stack's `.env`
  (`POSTGRES_USER`), not a rename in the repository, and 0132 T2 deals with the roles.
...
4. **Merge, then the upgrade gates against rc.1**, as `release.md` asks:
   - `git fetch origin tag v0.1.0-rc.1`, and the shared-chain gate's five tests *executed*;
...
5. **Pick the commit.** The commit to tag is one the nightly managed gate on the OTA stack ran
   green, since that gate keeps running for `main` (D2). That is the same condition 0131 T5 sets
...
- **ADR-0045's operative rule** is amended in place (ADR-0038). Pre-release ended with the alpha tag
  on its date, and from then on both chains only grow. `how-migrations-are-authored.mjs` line 95
  follows it.
- **No migration a release shipped may change.** From the alpha tag on, every file of either chain
  at a release tag must be byte-identical at HEAD. That covers squashing, editing in place (what
  ADR-0036 did to rc.1's baseline) and deleting, and it covers the managed chain, which no gate
  compares with a release today.
...
- For both chains, every file at the tag in `release-line.json` is byte-identical in the working
  tree. The tag must be reachable under `CI`, and the test fails hard if it is not; locally it
  warns, as the existing upgrade gate does.
...
- `scripts/every-release-is-a-starting-point.unit.test.ts`: under `CI`, the newest `v*` tag in SemVer
  order is in `release-line.json`'s list. It fails today, because the file does not exist. Once
  the file lists rc.1, it passes while rc.1 is the only tag, and fails the moment the alpha tag is
  pushed without the alpha leg below, which is when it matters.
...
The plans that record a sitting on the live stack also record the tag it ran against: 0141's walks
and records (*"which commit C was: 0146"*), and 0143 T9's measurement.
...
- **names the task deployment after the release:** it passes `--external-id <tag>` to the task
  deploy. The 4.5.16 CLI has that flag. Whether the self-hosted server of the same version records
  it was not checked. The first live deploy shows it, and the flag is dropped if the server ignores
  it;
...
**Order.** The change lands on `main`, and the nightly gate deploys the tasks on the OTA stack
under `node-24`; that is the proof on a managed stack, with demo data. The alpha tag then includes
it. `ownpace-live` starts fresh at the tag, so if T6 lands before the tag, no tester's pass ever
runs on Node 21.
...
**Guard 2: a pull request shows the bundle loads.** `scripts/a-task-bundle-the-runtime-can-load.mjs`:
...
- **(a) Accept it for the alpha, in writing.** *Recommended for the alpha's few weeks.*
  - Why it is tolerable:
    ...
    - its password is not the shipped one: live generates its own (0132 T1b), and 0132 T2 replaces
      the OTA stack's;
    - 0136 keeps a host that a tester types away from the compose service names.
...
and the same line goes into 0119 T3's item 5.
...
**Before the first invitation: T0, T2, T5 and T6.**
...
- **T6**, because otherwise every tester's pass runs on Node 21, which no test covers.
...
- a problem report sent from `ownpace-live` carries the build line;
...
- Being told when a watch opens an issue, or a deploy fails: 0142.
