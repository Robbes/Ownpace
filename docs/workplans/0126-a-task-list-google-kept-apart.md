...
still stand between this and a live run. **T3's probe:** the connection Test's headline check still
lists the calendar face, so a grant for Tasks alone fails it. **The Google project:** the owner
enables the Google Tasks API and declares `tasks.readonly` on the consent screen, and the console
says which class it is in. It is believed sensitive, as calendar and carddav are, but only the
console can confirm that.
...
| T3 The Test and the preflight | 🟡 **Half: the measure landed with T2; the probe is next** | Built with T2, because widening `GoogleGrantDomain` forced it: `qualifyGoogleGrant` measures the face (lists and tasks counted, deleted ones not), and `GOOGLE_NO_TASKS` is gone. The preflight follows the same builder seam (`run-discovery` → `buildTaskSourceFromConnection`). **Still open:** the connection probe (`probe-connection.ts`) always lists the CALENDAR face, so a grant for Tasks alone fails the Test. Microsoft's lists the first face the grant carries. |
| T4 The screens and docs | 🟡 **Screens done in T2; docs open** | Done with T2: the wizard card and its restricted twin (en, nl), the setup step naming the Tasks API, the Connections tick (read from the table), the scope manifest, the OpenAPI text, the `sourceDomainRefusal` sentence, and the tests that pinned "a Google account offers no Tasks tick". **Still open:** the docs. That means the feature matrix, the scope class in `docs/google-oauth-verification.md` (read from the console once the scope is declared), the API, scope and domain-wide delegation tables in `docs/google-workspace-setup.md`, and `docs/managed-bring-up.md`. It also means the archive qualification's word for Google's tasks, which is Apple's ("reminders"). |
