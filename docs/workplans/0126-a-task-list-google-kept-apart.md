...
It is built after T8, because the live run shows what Nextcloud Tasks actually displays. See the
T7 row for the shape, and for the one check that comes first: reminders may need converting
rather than counting.

**2026-09-23, the Google project is done.** The owner enabled the Google Tasks API and declared
`tasks.readonly`, and the console filed it under **Your sensitive scopes**: sensitive, like
calendar and carddav, so Tasks stays a default face (`docs/google-oauth-verification.md`). What is
left for T8: reconnect the account with Tasks ticked, add Tasks to the running migration (0125 T6,
#1097; a migration's data types used to be fixed at creation), then the live run.

**2026-09-23, T1–T6 are on main** (#1088, #1090–#1093). A Google account can carry Tasks
from the consent to the target, and both task sources' lists arrive as task lists. Nothing
left in code stands between this and T8. What is left is the owner's: **the Google project**
(enable the Google Tasks API, declare `tasks.readonly` on the consent screen and read its
class), **a reconnect** of the account with Tasks ticked, then **T8** on that account. **T7**
waits on the owner's answer to one proposal: on the confirm screen, one count per reason a
task's detail cannot carry.
...
| T6 The target makes a TASK list | 🟢 **Done for both sources 2026-09-23**; one question left for T8 | `CalDAVTargetWriter` sends `supported-calendar-component-set` on MKCALENDAR only when the source folder declares components. Google's lists declare `VTODO` (T1), and Microsoft To Do's now do too (`graph-todo-source.ts`, pinned in `a-microsoft-to-do-list-arrives-as-vtodo.unit.test.ts`). Only a collection the target CREATES is affected: an existing one is found and reused, so nothing already migrated changes. **Still unchecked:** what Nextcloud makes of an MKCALENDAR with no set. That now matters only to a source that declares nothing, and a live Nextcloud answers it (T8). |
| T7 Say what does not carry, for BOTH sources | 📋 Open; **shape confirmed by the owner 2026-09-23**, built after T8 | § The comparison. Google cannot give recurrence or a time of day. To Do drops attachments, linked resources, reminders not in UTC, unknown recurrence types and checklist structure, and none of it is reported today. On par means both say so, in the same shape. **The shape (D4, confirmed):** Google's limits cannot be counted, because the API does not show them, so they are one fixed sentence: the scope manifest's, repeated on the confirm screen of a Google migration that has Tasks. To Do's can be counted, so the confirm screen gets one count per reason beside the refused-files count: attachments, linked resources, repeat patterns that cannot be translated, reminders Graph gives in a zone other than UTC, and checklists (which arrive as text in the description). **Attachments get the same tick-box as refused files**, because that is content left behind; the rest is information only. **One check first:** `utcStampOfZoned` (`graph-todo-source.ts`) drops every reminder whose zone is not UTC. If Graph gives reminders in the user's own zone, that is most of them, and the answer is to convert them, not to count them. |
| T8 Live proof | ⏳ **Unblocked in code 2026-09-23**; the Google project is done, and what is left is a reconnect with Tasks ticked and Tasks added to the migration (Status above) | On the owner's account: the preflight counts tasks, including completed ones (the `showHidden` trap), subtasks arrive nested in Nextcloud Tasks, a second pass creates nothing, an edit in Google reaches Nextcloud, and Mailpit stays silent. |
