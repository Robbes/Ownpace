...
| T0 The probe becomes product | ✅ **Done 2026-08-26** | The 0103 T3 remainder, home at last. `measureTargetScheduling` (orchestration `target-scheduling.ts`) wraps the probe and runs at every DAV-target connection test: the verdict rides the probe result as a structured `scheduling` field AND appended to `detail` (carddav skipped — nothing to measure; UNKNOWN worded unmeasured-is-not-safe). **It stopped rendering at the connection test on 2026-09-07, on the owner's instruction:** *"people before migrate don't think of such issues: they assume we know our business, so this suppression and possible issues don't need to be addressed here. it would be an issue if we don't suppress such things"* — raising a hazard on a connection card in order to defuse it teaches a reader to worry about something the writer has already handled unconditionally. `schedulingText` and its six strings went with the three lines that called it. NOTHING ABOUT THE MEASUREMENT CHANGED: it still runs at every DAV-target test, still rides `detail` (so the appliance report and every `detail` consumer still carry it), still lands in `calendar.target_scheduling`, and the assessment report still opens with it — which is where a person meets it deliberately rather than beside the word Connected. Before a mapping's first calendar write, `schedulingRecorder` — wired in BOTH deps builders — measures the SAME endpoint the writer got and records `calendar.target_scheduling` to the audit log: once per mapping (the audit log is its own guard), ordered before the loop (proved by breaking the order), never a pass-killer. The migration assessment (the §14.2 permission report, both editions) now opens with "What the target will do with what we write", measured live at report time; a failed measurement is a stated unmeasured-is-not-safe line, never a dropped section. The appliance has no interactive test button — its verdict arrives through the report and the audit record, the halves a headless edition can carry. |
