...
| T2 The operating screens | 🟡 **First slice built, PR open** | The shared layer speaks Dutch: the queue **primitives** (domain tags, evidence-badge titles, guidance summary, receipt lifecycle prose incl. the `JobFailed` prefix — with REFUSALS deliberately left as the server's verbatim words, rule 2, now pinned by a test), the `QueueScreen` lifecycle note (key-based `LIFECYCLE_NOTE_KEY`), and the **`MappingDetail` hub** (five names + blurbs, fallback title, no-id and degraded-detail messages). All 13 existing web suites pass unmodified (default EN); an NL spot test proves Dutch rendering + the verbatim refusal. Second slice (PR pending): **Moves and Failures screen bodies** — titles, intros, section headings (shared `queue.waitingOnYou`/`queue.alreadyDecided` keys), empty-state texts, action labels (keep/retry/accept) and the try/tries plural, with `lastError` and all guidance prose still the server's verbatim words. Third slice (PR pending): the **Deletions screen body** — title, intro, section headings (Watching + the shared keys), empty states, the keep label, and the destructive button's label + armed label in Dutch; the transport-failure fallback message keyed; the server's refusal words and `whatThisMeans`/guidance verbatim as ever. Fourth slice (PR pending): **Confirm and Verify bodies** — Confirm's title/intro/start button/lifecycle notes/error prefixes (server error messages verbatim) and Verify's full surface: status hover-help ×5 (the status WORD stays server vocabulary), domain labels reusing the shared `domain.*` keys, the checksum-cell fragments, not-measured + its honest title, ready/not-ready banners, all seven table headers, Issues/What-to-do headings, run buttons, the duration hint and the three error messages — with `issues[].message` and `recommendations` (server prose) verbatim. Fifth and final slice (PR pending): the **Finish body** — the whole five-step checklist speaks Dutch (lifecycle notes, step titles/bodies, queue-count link phrases, the pass-state trio, step 4's warning + attestation checkbox, step 5's nothing-changes framing, the force button offered only after the refusal, the finish button + its disabled title, unknown-mapping and read-error messages, the left-unmigrated plural) — with the server's refusal `error`/`hint`, `effect` and `ifYouNeedToResume` verbatim as ever. **With this, every screen T2 names is bilingual: primitives, hub, Deletions, Moves, Failures, Verify, Finish, Confirm.** |
