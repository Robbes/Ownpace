...
**Still not measured, and still not to be assumed:** a Drawing under any policy; a Sheet or Slide under `export-odf`; a content-rich deck under `export-pdf`; any of this on a second tenant or day. Every `export-pdf` green is still five identical draws on one document of its type, and a green is not the mirror of a red. |
...
- a **content-rich deck** under `export-pdf`. The deck measured was 2017 bytes; images, embedded
  fonts and charts are the surface a PDF renderer is known to vary on.
