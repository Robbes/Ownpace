...
| T10 the holes the formats leave | 🟠 **Found 2026-09-23. A fix is proposed; two owner decisions are open** | **(a) A Google document renamed in Drive is reported as deleted in Google.** Proved with a throwaway probe through the real sync loop, not committed. `detectPathKeyedMoves` pairs a disappearance with an arrival by `contentHash` alone, and a zip export (`.docx`, `.xlsx`, `.pptx`, `.odt`, `.ods`, `.odp`) differs byte for byte between two exports of the same document. So a renamed Doc is not seen as a move: after two passes the old name is a confirmed `inferred` deletion at the source, and both copies stay on the target. PDF and SVG exports are byte-identical, so a rename under those is reported as a move, as it should be. The owner's live migration exports as Office, so it is affected. **Proposed:** pair a Google document's disappearance and arrival by its Drive id (`sourceRef`), which a rename does not change. That changes reporting only: "moved" instead of "deleted in Google". **Owner decision open:** may renamed Google documents be paired by Drive id? **(b) Removing the old copy of a renamed document** is ADR-0030's Apply, and one of its gates is that the arrival's bytes match. A new export never matches, so Apply would refuse. **Proposed:** for this case, a gate that holds the same line by other means: the new copy is the current export of the same Drive document, written by us, and on the target. That amends ADR-0030, and is the owner's decision. **(c) The two refusals may be liftable after (a).** They were made because an export that differs byte for byte would be copied again on every pass. Since #1083 a native document is copied again when Drive's modified time moves, not when its bytes do, so that reason no longer holds by itself. (a) is the one place found so far that still depends on the bytes, so lifting the refusals of Docs under ODF and Slides under Office waits on it. |
