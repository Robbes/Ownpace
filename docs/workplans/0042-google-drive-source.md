...
| T6 proof against a real Drive | 🟡 **The recorder is built and tested; the recording needs the owner's tenant** | `scripts/drive-export-stability.ts` with `DRIVE_CAPTURE_FILE` set produces, in ONE run, both the T0 Q3 byte-stability verdict and a **redacted** fixture for the replay tier — names, ids and page tokens become pseudonyms, export bodies become a sha256 and a length, and everything structural stays verbatim (`packages/testing/src/drive-capture.ts`, 9 tests, each naming a specific thing that must not survive). Chosen over a manual-dispatch e2e because a recording gates every PR while a nightly against a live tenant gates whichever PRs happen to run at night. **What it will not cover, stated now rather than discovered later:** anything Drive changes after the day it was recorded, and anything about the owner's specific documents — the verdict speaks to that and is printed, not stored. |
