...
| T7 build ADR-0046: a rendering compared by its parts | 🔴 **Not started — the decision is made, the code is not** | [ADR-0046](../adr/0046-a-rendering-is-compared-by-its-parts.md) was accepted by the owner 2026-09-16 on the T3 measurement: `export-office` is byte-unstable ONLY in its zip container, all nine members byte-identical across five draws. Until this row is done the tree still hashes every file whole, so the ADR states the gap in its own operative block rather than reading as a description of the code. **Four pieces, and the fourth is the one that bites if it is skipped:** (a) a canonical container hash — member names sorted, sha256 of each member's UNCOMPRESSED bytes, zip bookkeeping excluded — reachable only for a rendering this product asked `files.export` to produce, never for a `.zip` a customer stored; (b) **sha256 of inflated bytes, never the stored CRC-32** — the diagnostic uses CRC-32 because it is free, and 32 non-collision-resistant bits must not decide whether a customer's file is rewritten; (c) §20's report saying, for those rows, that the comparison was over the container's parts; (d) **a scheme tag on every stored hash, and no comparison across schemes** — a row whose scheme differs is NOT evidence of change, it is recomputed and stored without a re-copy. Skip (d) and adopting the scheme rewrites every already-migrated native file exactly once, which is the disease this workplan exists to prevent arriving through its cure. Each piece wants a test proved by breaking it; (d) especially. **Enabling `export-office` is still a separate owner decision** and still wants a Sheet and a Slide measured. |
