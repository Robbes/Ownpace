...
> **In one line:** The Google Drive file source: `GoogleDriveSource`, its config and OAuth wiring on both editions, export formats per kind for native Docs, Sheets, Slides and Drawings, export byte-stability measurements, and rename pairing by Drive id.

## Status — 2026-09-23 (update this block at the end of every session)

**2026-09-23, later that night: T10 (c) BUILT: every format carries every kind.** The refusals
of a Doc under OpenDocument and a Slides deck under Office went. They protected against a rewrite
on every pass, and nothing depends on the bytes any more: a document is copied again when Drive
says it was edited (#1083), and a renamed one is paired by its Drive id (T10 (a)). The owner's
aim was *"working fileformats that suite the user"*. The measurements stay in `EXPORT_STABILITY`
as a record, the per-kind selects offer all three formats for every kind, and ADR-0046 is
amended. Decks already refused on the owner's live migration stay on the Failures screen until
somebody presses Try again (per row or per group), as a parked refusal always has.

**2026-09-23, night: T10 (a) and (b) DECIDED and BUILT: a renamed Google document is paired by
its Drive id.** The owner: *"Yes, pair renamed Google documents by their Drive id"* and *"Yes,
Apply may remove the old copy of a renamed Google filetype/document."* A rename under Office or
ODF is now reported as a move, never as deleted in Google, and Apply may remove the old copy when
the new one is the same Drive document, written by this migration and on the target. A rename
from before this, already reported as deleted, is paired on the next pass and its report taken
back. Unattended apply leaves those pairs for a person. PDF and SVG renames stay bytes pairs, as
before. ADR-0030 is amended. Lifting the two refusals (T10 (c)) is next.

**2026-09-23, evening: T9 DECIDED, and its engine BUILT: a format per kind.** The owner:
*"a per kind choice makes more sense for the fileformats. Split that up, and we also need to
further check if we can fix the wholes."* A mapping can now give Docs, Sheets, Slides and Drawings
each their own format, laid over the single one, on both editions. The screens followed: the
wizard and the migration page ask per kind, each select offering only the formats measured to
carry that kind, and one press sets an editable format for every kind. Checking the holes found
one that affects the
owner's live migration (T10): a Google document renamed in Drive is reported as deleted in Google,
because an Office or ODF export differs byte for byte between two exports and moves are paired by
bytes. The proposed fix pairs them by Drive id, and waits on two owner decisions (T10).

**2026-09-23, later: T8 (b)'s second half is built: a copy the old format left is an earlier
export, never a deletion.** After a pass that listed everything, our own copy (`copied`,
`updated`) under a name the document no longer has is marked as an earlier export of its current
row (`markEarlierExports`, matched by Drive's file id where the row has one). The mark reuses
migration 0056's `superseded_by_natural_key_hash` on a row that stays `copied`, so no migration
was needed. The deletion detector leaves a marked copy alone, so two clean passes no longer report
every old copy as deleted in Google. The Deletions screen lists them in their own section, *Earlier
exports*, with **Keep**, and they count in none of the deletion numbers: counted as deletions, a
format switch would raise the mass-deletion breaker for every real removal in the file domain. The
export-format setting now says before a save that every Google document is copied under its new
name and old copies stay, listed as earlier exports. Removing an old copy is by hand on the target
for now; a remove button for earlier exports would be its own change. **The owner can now switch
the format on his live migration**, bearing in mind the finding below about which kinds each format
carries.
...
| T8 what the first live run left open: Slides, suffixes, and an edit nobody copies | 🟡 **Listed 2026-09-22. (d) FIXED by #1083; (c) DECIDED 2026-09-23: keep; (g) DONE 2026-09-23. (b) DECIDED 2026-09-23 and BUILT the same day, both halves** | Each point was checked against the source on 2026-09-22. **(a) Slides under `export-office` are refused, on purpose** (lifted 2026-09-23 by T10 (c)). T3 measured five parts of the `.pptx` changing between draws, beyond the zip wrapper. Nothing is built to change that: it needs either an ADR declaring those parts not the document, or a change signal that is not the bytes (see (d)). `export-pdf` is measured stable only on a thin deck; `export-odf` is stable. Under the default `refuse` a deck is `policy_refused`. **(b) Switching the export policy renames every native file.** `exportedName` appends the export's suffix, and the name is part of the natural key, so under `export-office` a Doc listed as `Report` becomes `Report.docx`: a new item. The comment calling that *"survivable only because … no deployment has selected one"* is stale since 0125 made the policy revisable on a live migration. The owner would see the files arrive under the new names while the rows refused under the old names stay on the Failures screen for good, because nothing lists those keys again. A Slides deck that is still refused under the new policy appears twice. Between two export formats it is worse again: every document is copied a second time beside its old copy, and because a Drive pass lists every folder, `detectPathKeyedMoves` sees each old copy's name missing with no arrival carrying its bytes (the new format's bytes differ), so after two passes each old copy is reported as an inferred deletion at the source. **DECIDED 2026-09-23 (owner: *"acknowledged what you adviced"*): one record per Google document, whatever its format.** Every row carries the document's Drive id (`source_ref`), so when a pass meets a Google document under a name the ledger does not know, it looks for rows with the same Drive id under another name. A refused or failed one is closed as superseded by the new name, and it leaves the Failures screen by itself. An earlier export that is on the target is never deleted for the owner. It is offered in the Deletions queue as an earlier export of the same document, not as a deletion at the source, and the owner decides. The export-format setting says all of this before a save: every Google document gets a new name, and earlier exports stay until they are removed. Until it is built, the owner holds off switching the format on a live migration. **First half built 2026-09-23: the refusals.** Matched by name rather than by Drive id alone: the Drive source lists each native document's paths under the other policies (`formerPaths`, the same `exportedName` rule), the file pass hashes them, and after a pass that listed everything, `supersedeFormerNames` closes a `failed` row under a former name that pass did not list. A name still listed is left alone (a real `Deck.pdf` beside a deck called `Deck`), and so is a row whose recorded source handle is another object's; failures now record it. The status is `superseded` (migration 0056): off the Failures screen, out of `placedItems` and verification's source counts, never placed, and retried if the name comes back. **Second half built 2026-09-23: earlier exports on the target.** `markEarlierExports` runs beside `supersedeFormerNames` over the same unlisted former names and marks our own copy (`copied`/`updated`, never removed, source handle matching) with the row that took over; the absence count resets. `detectPathKeyedMoves` skips a marked copy and clears the mark when the old name is listed again (`clearEarlierExport`), so a switch back makes the other copy the earlier one. `listEarlierExports` feeds a new `earlierExports` section of the Deletions queue on both editions (`earlierExportsQueue`), and `keep` accepts one; `listDeletions` never contains one, so the mass-deletion breaker, the digests, the completion report and Finish are untouched. The setting's consequence line and `mayRevise`'s sentence say it before the save. **Evidence:** the pass (`a-name-the-document-outgrew`, core, 12 → 17): marked and nothing removed, never counted missing over three passes, the switch back, another document's copy and an adopted file left alone; the ledger on PGlite (10 → 22): the mark, the reset, idempotence, the chunking, `placedItems`, the clear, the listing, keep, the order; the breaker (`apply-deletion`, 31 → 32): nineteen earlier exports and one reported deletion, applied; the page (4 new); the appliance's queue route. **(c) Doubled extensions: DECIDED 2026-09-23, keep.** The suffix is skipped only when the name already ends in that same suffix, so a Sheet named `Budget.xls` becomes `Budget.xls.xlsx` and a Doc named `Notes.rtf` becomes `Notes.rtf.docx`. The owner: *"if those are the original files and work in the target, we keep"*. Both conditions hold. Only a Google-native document is renamed, and the name the owner gave it stays whole in front of the suffix; an uploaded `Budget.xls` is copied byte for byte under its own name. Nextcloud types a file by its last suffix (`Detection::detectPath` in `nextcloud/server`, read 2026-09-23), and that suffix is the format the export actually is. The tests pinned only the same-suffix case; `a-document-that-arrived-without-its-extension.unit.test.ts` now pins this one too. **(d) FIXED by #1083 (2026-09-22): a Drive file edited after its first copy was never copied again, and the same held for OneDrive.** The file domain's change signal is `sourceVersion`, read from `FileItem.etag` (`dav-sync.ts`), and `classifyKnownItem` answers `skip` when it is absent. Neither Drive's `toFileItem` nor the Graph drive listing set it; only the WebDAV source did. It is also why no unstable export was ever rewritten nightly: nothing was rewritten at all. Box and Dropbox had the same gap. All four now fill it through `fileVersion` (`packages/shared/src/file.ts`): the provider's content hash where it keeps one (Drive `md5Checksum`, OneDrive `quickXorHash`, Box `sha1`, Dropbox `content_hash`), and the last modification otherwise. That is not the `version`/`eTag` first planned here, because those move on metadata alone (Drive's `version` *"reflects every change made to the file"*, and Graph's `eTag` covers *"metadata + content"*), so a rename would re-copy unchanged bytes. A native Doc has no checksum, so its version is its `modifiedTime`, the only sign of an edit it shows before it is exported. **(e) ADR-0046's per-part hash does not reach the ledger.** Drive's `fetch` marks an export `rendering: true`, but `dav-sync`'s `fetchRaw` rebuilds the raw item from `item`, `content` and `body` only, so `contentHash` sees no flag and hashes the whole file. The guard test for it reads source text, not a stored hash. This is harmless while (d) holds, and once versions arrive a version decides a rewrite, not a hash. ADR-0046 and its register row should still say what is actually live. **They do since 2026-09-23**: the status line, the first operative rule and the register row now say built but not reaching the ledger. The code half stays open: whether to pass the marker through `fetchRaw`, and what that changes for relocation and verification, which compare stored hashes. **(f) An export has no size until it exists**, and it is always buffered. Drive's own export size limit (`exportSizeLimitExceeded`) has no fallback, and image-heavy decks are the likely casualty. Not seen live. **(g) Stale docs: DONE 2026-09-23.** `docs/feature-matrix.md`, `docs/selfhost-quickstart.md` and `docs/owner-test-runbook.md` called the exports unmeasured, and the ADR register called ADR-0046 unbuilt. All four now say what was measured and what is live. **Order:** (d) first, because it was data going stale silently and it changes what (a) and (e) need: done. A deck is now re-exported only when its `modifiedTime` moves, which is the change signal that is not the bytes that (a) names, so whether the refusal still protects anything is (a)'s next question. Then (b), before any owner switches policy on a live migration. (c) is decided and needs nothing built. (a) and (f) need live measurement. |
| T9 a format per kind: Docs, Sheets, Slides and Drawings each their own | 🟢 **DECIDED 2026-09-23 by the owner, and BUILT the same day: the engine on both editions, then the screens** | The owner: *"a per kind choice makes more sense for the fileformats. Split that up."* No editable format carries all four kinds (the T8 finding): ODF refuses every Doc and Office every Slides deck. **Built:** a mapping's source takes `nativeFilePolicies`, a format per kind (`document`, `spreadsheet`, `presentation`, `drawing`), laid over `nativeFilePolicy`. A kind left out follows `nativeFilePolicy`, so every mapping written before reads as it did. One function, `nativeFilePoliciesOf`, answers which format each kind is exported in, for the connector and for the revision snapshot. The connector decides each file under its own kind's format: the name, the export, the refusal, the preflight's count and the names the document had before. The shared parser refuses an unknown kind or format by name, on both editions. The revision snapshot keeps its one field: it records the single format when all four kinds agree, so no stored snapshot changes, and one format per kind when they differ. The managed create and update routes store it in the mapping's own override and treat the two keys as one setting: choosing one format for all four writes the per-kind ones as none, so a reused connection's per-kind formats cannot win over it. The update route now answers an unreadable format with a 400 naming the key; it answered 500. **The screens:** the wizard and the migration page show one select per kind. Each offers "leave behind" and the formats measured to carry that kind, once per file (a Drawing's `.svg` is one entry under both document families); a format in force that leaves its kind behind is shown as left behind rather than hidden. The lines under the selects name the kinds left behind and the kinds arriving as a PDF nobody can edit, from the same tables. One press (*Use an editable format for every kind*) sets Office where it carries a kind and OpenDocument where it does not. Both screens send all four kinds, and the migration page reads each kind's own format through `nativeFilePoliciesOf`, as the engine does. The refusal of a measured-unstable pair now says to choose a format for that kind under *Export format for Google files*. **For the owner's live migration:** Office for Docs, Sheets and Drawings and ODF for Slides leaves nothing behind and keeps every kind editable. |
| T10 the holes the formats leave | 🟢 **Found 2026-09-23. (a) and (b) DECIDED by the owner the same day, and BUILT; (c) BUILT the same night** | **(a) A Google document renamed in Drive was reported as deleted in Google.** Proved with a throwaway probe through the real sync loop, not committed. `detectPathKeyedMoves` paired a disappearance with an arrival by `contentHash` alone, and a zip export (`.docx`, `.xlsx`, `.pptx`, `.odt`, `.ods`, `.odp`) differs byte for byte between two exports of the same document. So a renamed Doc was not seen as a move: after two passes the old name was a confirmed `inferred` deletion at the source, and both copies stayed on the target. PDF and SVG exports are byte-identical, so a rename under those was reported as a move, as it should be. The owner's live migration exports as Office, so it was affected. **DECIDED 2026-09-23 (owner: *"Yes, pair renamed Google documents by their Drive id"*). BUILT the same day:** the Drive source gives a Google document its Drive id as `FileItem.sourceIdentity`, and `runFileSync` hands it to the detector through a new `identity` hook, noting every name each document is listed under. Before it looks at bytes, `detectPathKeyedMoves` pairs a disappeared row whose recorded `source_ref` is listed this pass under another name with a copy on the target. Asking what is listed now, not only what arrived, also pairs a rename the pass did not see happen: one from before this was built, already reported as deleted, or one whose new name failed its first copy. The absence and any deletion report are cleared when it is paired. Where the two exports' bytes still match (PDF, SVG), the move is recorded as a bytes pair, exactly as before, so nothing changes for those. Where they differ, it is recorded with `moved_by_identity` (migration 0058). Google documents' arrivals are kept out of the bytes index. A format switch is still an earlier export (T8 (b)), never a move. **(b) Removing the old copy of a renamed document** is ADR-0030's Apply, and one of its gates was that the arrival's bytes match, which a new Office or ODF export never does. **DECIDED 2026-09-23 (owner: *"Yes, Apply may remove the old copy of a renamed Google filetype/document."*). BUILT the same day, and ADR-0030 amended:** for a pair made by id, Apply asks for the same Drive document (same `source_ref`), written by this migration (`copied`/`updated`) and present on the target (asked, as for every relocation), in place of the same bytes. The ledger's own re-check under the write (gate 7) accepts exactly that, on both editions. A pair made by bytes still owes the same bytes, whatever ids it carries. Unattended apply (ADR-0031) leaves a pair made by id for a person, with the reason `paired_by_identity`. The Moves screen's Apply guidance says it. **Evidence:** the pass (`a-rename-the-bytes-could-not-pair`, core, 20): reported as moved and never as deleted over three more passes; another document not paired, not even by byte-identical exports; every old name the document left paired with its current one; a rename from before this, already reported as deleted, paired on the next pass and its report taken back; a new name whose first copy failed paired once it is copied; a rename back after the first move was applied paired the other way; the pair and how it was made forgotten when the old name comes back; a PDF export still a bytes pair that unattended apply acts on; a format switch not a move; a binary still paired by bytes, and not by id when it was edited too; Apply removes, and refuses a changed id, two blank ids, an adopted arrival, an arrival missing from the target and a bytes pair whose bytes differ; unattended apply leaves the pair. The connector (same name, 2): each of the four kinds listed with its Drive id, and a stored file without one. The ledger on PGlite (same name, 11): the column written, read back, overwritten by a later bytes pair and cleared with the move; `placedItems` hands over the id; gate 7 removes for the same document and refuses another id, a blank id, an adopted arrival and a bytes pair with other bytes. **(c) The two refusals: BUILT 2026-09-23, lifted.** They were made because an export that differs byte for byte would be copied again on every pass. Since #1083 a native document is copied again when Drive's modified time moves, not when its bytes do, and (a) was the one place found that still depended on the bytes. So the connector no longer refuses on `EXPORT_STABILITY` (the table stays, as a record, and the instrument's private way past the refusal is gone with it), `NATIVE_POLICY_COVERAGE` has every kind under every format, and the per-kind selects offer all three. The owner's aim: *"working fileformats that suite the user"*. ADR-0046 amended. **Evidence:** `a-deck-copied-once-not-nightly` (core, 3), through the real Drive connector and the real file pass with an export that differs on every fetch: a deck under Office and a Doc under ODF are copied, not copied again over three more passes (not even exported), and copied again once when Drive's modified time moves. The coverage guard now asks the connector what it refuses rather than deriving it from the measurements; the chooser keeps a test, with a table that has an empty cell, of what it says when a format drops a kind. |
