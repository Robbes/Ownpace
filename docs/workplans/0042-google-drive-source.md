...
**Still not measured, and still not to be assumed:** a Drawing under any policy; a Sheet or Slide under `export-odf` or `export-pdf`; any of this on a second document, tenant or day. The `export-pdf` green remains the weakest result here — five identical draws on one Doc — and a green is not the mirror of a red. |
...
**ONLY THE WIRING IS OWED, and the Slide narrowed it.** Nothing calls `containerContentHash` yet. It is two-sided and must land as one change: the source's export branch must produce `zip1:`, and the target re-read (`webdav-target-writer.ts:582`, a whole-file sha256 today) must be told by the row's own scheme to answer in the same one. With only the source half, rule (d) makes every exported Doc read `unavailable` — honest, and no use. **And per the T3 measurement above it will rescue a Doc and a Sheet and NOT a Slide**, so the wiring is not the last step before `export-office` becomes enableable; a policy that is safe for two editor types and not the third is a product question, not a hash question. **Enabling `export-office` is still a separate owner decision** and still wants a Sheet and a Slide measured — the 2026-09-16 attempt measured the Doc three times instead, because `DRIVE_FILE_KIND` had not merged and the variable was ignored. |
