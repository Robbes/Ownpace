...
it. Which store the managed builder, the probe and the wizard use is not wired: a managed
archive connection has no target until a migration names one, so where its Test measures is
the owner's to say.
