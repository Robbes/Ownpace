...
Graph is in #874 (which also carries a defect found while reading it — that
connector was text-decoding file bodies, destroying every OneDrive file that
was not valid UTF-8) and Dropbox in #875. **All three touch the T5 row below**,
so whichever merges last needs a one-line resolution: keep the row naming every
connector done so far. Box and the archive source still buffer.

**2026-09-08, later: T5 one connector per PR.** Box is done here; Graph is in
#874 (which also carries a defect found while reading it — that connector was
text-decoding file bodies, destroying every OneDrive file that was not valid
UTF-8), Dropbox in #875, Google Drive in #876. **All four touch the T5 row
below**, so whichever merges last needs a one-line resolution: keep the row
naming every connector done so far. Only the archive source is left, and it is
the one that may legitimately stay buffered — see the row.

**2026-09-08, later: T5 is complete, one connector per PR.** The archive source
is done here; Graph is in #874 (which also carries a defect found while reading
it — that connector was text-decoding file bodies, destroying every OneDrive
file that was not valid UTF-8), Dropbox in #875, Google Drive in #876, Box in
#877. **All five touch the T5 row below**, so whichever merges last needs a
one-line resolution: keep the row naming every connector. Two of the five were
not the shape the row assumed, and both are recorded there: Drive cannot stream
an export, and the archive source turned out to be the CHEAPEST rather than the
hardest.
...
| T5 | The other five connectors | ✅ **Done 2026-09-08** — all five: Graph in #874, Dropbox in #875, Drive in #876, Box in #877, the archive source here | The seam is what made them independent: each connector moved to `FileBody` on its own and nothing above it changed. **A correction to this row as written:** these are all SOURCES — nothing in this system writes files to OneDrive, Dropbox, Drive or Box (the file targets are `webdav-target-writer.ts` and `jmap-file-target.ts`) — so the work per connector is `fetch` returning a body above `STREAM_FILES_LARGER_THAN_BYTES`, not a resumable upload protocol. Upload sessions become the subject only if one of them is ever a target. **Dropbox, done:** `fetch` returns a `FileBody` above the threshold; the download moved into its own `download()` so `open()` issues a fresh one every time — a retry starts from the beginning and a consumed stream cannot — and a 200 with no body is refused rather than written as an empty file and recorded as a copy. Its bytes were never in question: this connector already read `arrayBuffer()`. What IS specific to it is that the download's argument travels in a HEADER (`Dropbox-API-Arg`, POST to `content.dropboxapi.com`), so a streamed read that rebuilt the request without it would get a 400 and read as a broken file rather than a broken request — the guard asserts that header on the streamed path, not only the buffered one. Guard: `a-dropbox-file-nothing-holds.unit.test.ts`, 9 tests proved by five breaks (no streaming branch, `>=` for `>`, the wrong id in the header, an empty stream instead of the refusal, and `open()` reusing one consumed download). **Drive, done, and it is the one that is NOT the same shape as the others.** A native editor file has no size until the export exists: Drive reports no `size` for a Doc, so the listing carries 0 and the exported length is unknowable before Drive produces it. `FileBody.sizeBytes` must be honest BEFORE a byte is read — the daily byte meter spends it ahead of the fetch (0090) and the target promises it as `Content-Length` (T4) — so a guessed size is worse than a buffer. **An export is therefore never streamed**, and the size correction on the buffered path (`size: bytes.byteLength`) is not lost by streaming, because it exists for exports and exports still buffer. The metadata call also stays at `fetch` rather than moving into `open()`, unlike the Graph and Dropbox bodies which issue nothing until opened: the native-file refusal has to land inside the sync loop's per-item boundary, and whether the file can be streamed at all is what that same call answers. `DriveResponse` gained an optional `body`, like `DropboxTransport`'s. Guard: `a-drive-file-that-has-no-size-until-it-exists.unit.test.ts`, 11 tests proved by five breaks — no streaming branch, exports streamed too (the Drive-specific rule dropped), `>=` for `>`, an empty stream instead of the refusal, and `open()` reusing one consumed download. **Box, done:** the download moved into its own `download()` so `open()` issues a fresh one every time — and here that is not only about consumed streams: `/files/{id}/content` answers a REDIRECT to a signed, time-limited URL, so re-opening cannot mean reusing the URL from last time. A 200 with no body is refused by name rather than written as an empty file and recorded as a copy. The size correction on the buffered path stays: Box's listing asks for `size`, but when it is absent `toFileItem` reads 0, which falls BELOW the threshold and is corrected there — so a missing size can never produce a body that lies about its length. `BoxTransport`'s response gained an optional `body`, like Dropbox's and Drive's. Guard: `a-box-file-nothing-holds.unit.test.ts`, 8 tests proved by five breaks (no streaming branch, `>=` for `>`, an empty stream instead of the refusal, `open()` reusing one signed download, and the buffered path dropping the size correction). all five are SOURCES — nothing in this system writes files to OneDrive, Dropbox, Drive or Box (the file targets are `webdav-target-writer.ts` and `jmap-file-target.ts`) — so the work per connector was `fetch` returning a body above `STREAM_FILES_LARGER_THAN_BYTES`, not a resumable upload protocol. Upload sessions become the subject only if one of them is ever a target. **Two were not the shape this row assumed.** *Drive:* a native editor file has no size until the export exists, and `FileBody.sizeBytes` must be honest before a byte is read (the byte meter spends it, the target promises it as `Content-Length`), so an export is never streamed and the buffered path's size correction exists precisely for it. *The archive source, done here:* expected to be the hardest, because re-opening sounded like re-reading a zip entry per `open()`. It is the CHEAPEST of the five — `takeout-archive-reader` takes an EXTRACTED tree, so an item is a file on disk and re-opening is one more `createReadStream`: no second pass, no re-issued request, no signed URL to expire. `ArchiveReader.contentStream` is OPTIONAL, so a reader that cannot stream keeps the buffered path and `MAX_BUFFERED_FILE_BYTES` still refuses by sentence rather than by death; `content` and `contentStream` share one `locate` so the buffered and streamed reads cannot resolve to different files, which would be invisible since both would return plausible bytes. The synthesised manifest is returned before the threshold is consulted — small by construction. Guard: `an-archive-item-nothing-holds.unit.test.ts`, 6 tests proved by four breaks (no streaming branch, `>=` for `>`, streaming even when the reader cannot, and `open()` caching one consumed stream). |
