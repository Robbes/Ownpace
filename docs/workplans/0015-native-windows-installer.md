...
- **T3 install/upgrade/uninstall — the payload half is built.** `pnpm
  ...
  **The open list that stood here was answered by T3's own build (struck
  2026-08-10):** the data directory is `C:\ProgramData\OpenMigrate` (not
  `%LOCALAPPDATA%`), uninstall keeps the ledger deliberately, migrations run at
  boot before serving (same as every launch shape), and the payload ships its
  own checksum-verified Node runtime — "install Node first" never became an
  instruction. What genuinely remains Windows-side is in T3's row: the MSI
  question (owner) and signing (T4/owner).
