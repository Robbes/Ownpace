...
answer does not settle it, so it is now open question 4. #1194 merged on 2026-09-26, so the consent
asks for `account_info.read` and T7 (a) has its scope. T8 now has the owner press Retry before
...
once a format can be chosen (D1); the category it states before T3 is open question 4. T6 (b) was
...
D1 to D8 (*Decisions*). Four things stay open: T1's evidence (open question 1), the owner's own
words (open question 2), T2's listing of the owner's account, which decides open question 3 and is
needed before T3, and the category a Paper doc states before T3 (open question 4). Nothing is built.
...
no setting; which category it states then is open question 4. One part of the recommendation did
...
1. **An unstated category.** The bare `Error` from `download()` is classified from its prose and
   ...
   Before T3 there is no setting to change the answer, and which category a Paper doc states then
   is open question 4. A `.paper` listed above 8 MiB is downloaded inside the target's write, and
...
Paper doc states before T3. It is open question 4, not a decision.
...
to choose, and the answer does not say which category a Paper doc states then (open question 4).
...
- **`fetch()` decides by kind, before the size gate.** An exportable kind with no format chosen, or
  ...
  none, and its category is open question 4. A kind with no export path gets a stated
